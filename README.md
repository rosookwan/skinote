...
- **차량 기사:** 장갑을 끼고 차량 태블릿(1024×520 ~ 1280×720)이나 휴대폰(360×640)으로 배달 · 수거 목록을 봅니다. 산에서는 연결이 끊겨도 수거 처리(수거 도장)를 할 수 있어야 합니다.

그래서 화면은 글자 16px 이상, 누르는 곳 52px 이상(기사 기기 56px), 말줄임표와 스크롤 없이 쪽 넘김으로 만들고, 화면 글은 가게 · 포스에서 쓰는 짧은 업무 용어(장부, 접수증, 수령, 지급, 반납, 수납, 미수, 수거 목록, 긴급, 끝 4자리)로만 씁니다. 버튼은 `반납 처리 · 5개`처럼 동작 이름과 수 · 금액이고, 알림은 `처리 실패 · 재시도 필요`처럼 짧은 한 줄입니다. 말의 기준은 [화면 문구 표](docs/design/wording.md), 화면 규칙은 [AGENTS.md](AGENTS.md)에 있습니다.
...
- 찍은 도장과 수납은 그 브라우저에만 남습니다(다른 기기와 나누지 않음). `나가기` 화면에서 체험 시계를 앞으로 돌리거나(`+10분` · `+1시간`) `체험 자료 초기화`로 처음 자료로 돌립니다.
- 처음 화면에서 `카운터(포스)` · `기사 태블릿` · `기사 휴대폰`을 고릅니다. 지금 볼 수 있는 화면은 오늘 대여 장부, 대여 접수증과 처리 현황, 야간 수거 목록(기사 · 카운터)입니다. 새 접수 · 마감 · 관리처럼 아직 만들지 않은 화면은 `준비 중인 화면` 알림이 뜹니다.
...
| `packages/ui` | 화면 부품(장부 · 도장 · 접수증 · 처리 현황 · 숫자판 · 쪽 넘김 …), 기기 등급 `DeviceProfile`, 디자인 토큰, 묶은 글꼴(Pretendard) |
...
| `docs/design/redesign-mockups.html` | 고른 코드 시안: C 종이 장부와 도장에 A 지금 줄, B2 처리 현황(남은 일 목록), A3 ▲▼ 순서와 긴급 요청을 섞음 |
| `docs/design/wording.md` | 화면 문구 표: 대화형 말 대신 쓰는 짧은 업무 용어의 단일 기준(한 개념 한 말) |
...
- [화면 문구 표](docs/design/wording.md): 버튼 · 이름표 · 알림에 쓰는 업무 용어
