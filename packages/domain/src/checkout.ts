// V4 접수 확정 창 · 칸별 수납(spec 3-3, ui 4-5 · 6-4, catalog 15)의 읽기 모델(checkoutSheet)과 명령(order.create).
// 화면은 칸마다 누른 수단 · 할인 · 결제 팀만 인자로 다시 묻고, 칸 글 · 결제 팀 줄 · 받을 금액 줄 ·
...
function depositOf(state: ShopState, draft: OrderDraftInput, quote: Quote): PlanDeposit | null {
...
export function checkoutPlan(
  ...
  for (const pay of state.registry.paySections) {
    ...
    if (choice) {
      const method = payMethodOf(state.registry, choice.methodKey);
  ...
  if (deposit && depositChoice) {
    const method = payMethodOf(state.registry, depositChoice.methodKey);
    if (method && deposit.rule.methods.includes(method.key)) deposit.methodKey = method.key;
    else invalid ??= 'method';
  }
  for (const c of choices) {
    if (!sections.some((s) => s.pay.key === c.sectionKey) && c.sectionKey !== deposit?.rule.key) invalid ??= 'unsupported';
  }
  const payer = payerOrderId ? findOrder(state, payerOrderId) ?? null : null;
  if (payerOrderId && !payer) invalid ??= 'unsupported';
  ...
  return {
    registry: state.registry, draft, pickup, giveBack, quote, sections, deposit, payer, ...(invalid ? { invalid } : {}), itemsReady, scheduleReady, ...(reason ? { reason } : {}),
...
const methodLabel = (reg: ShopRegistry, key: string) => payMethodOf(reg, key)?.label ?? '';

/** 칸 제목 줄의 품목: 장비는 종류마다(`스키 4 · 의류 3 · 헬멧 1`, 하루를 넘기면 `· 2일`), 리프트권은 권종마다(`야간권 성인 4매`). */
function itemList(reg: ShopRegistry, s: PlanSection, days: number): string[] {
 ...
 */
function linkedTeams(state: ShopState, payer: FxOrder | null, found: readonly string[] | undefined): FxOrder[] {
...
function sectionView(plan: CheckoutPlan, s: PlanSection): CheckoutSection {
  ...
  const quick = plan.registry.payMethods.filter((m) => m.quick);
  ...
  const items = itemList(plan.registry, s, plan.quote.days);
  return {
    key: s.pay.key,
    label: s.pay.label,
    items: items.join(' · '),
...
function depositView(plan: CheckoutPlan, d: PlanDeposit): CheckoutSection {
  ...
  return {
    ...
    discountable: false,
    ...(two
      ? { methods: d.rule.methods.map((key) => ({ key, label: methodLabel(plan.registry, key), selected: d.methodKey === key, enabled: true })) }
      : { single: [{ text: methodLabel(plan.registry, d.methodKey) + ' ' + won(d.amount), strong: true }] }),
    choice: plan.choices.find((c) => c.sectionKey === d.rule.key)!,
  };
}

/** 지금 받을 돈(수단마다 합, 매장 목록의 수단 차례)과 그 구성(칸 · 보증금). */
function dueOf(plan: CheckoutPlan) {
  ...
  const byMethod = plan.registry.payMethods.map((m) => ({ key: m.key, label: m.label, amount: all.filter((p) => p.methodKey === m.key).reduce((sum, p) => sum + p.amount, 0) }))
    .filter((m) => m.amount > 0);
...
export function checkoutSheet(ctx: ViewContext, params: CheckoutSheetParams): CheckoutSheetView {
  ...
  if (due.total === 0) dueLine = [{ text: '받을 금액 없음', strong: true }];
    ...
    dueLine = [
      ...
      ...(due.deposit ? [{ text: ' (' + methodLabel(plan.registry, due.deposit.methodKey) + ' 중 보증금 ' + won(due.deposit.amount) + ')' }] : []),
    ];
  }
  const base = '접수 확정';
...
export function createOrder(
  ...
  if (giveBack.at === undefined) return unsupported(domainLines);

  const seq = state.nextReceiptSeq;
  const receiptNo = state.businessDate.slice(2).replace(/-/g, '') + '-' + String(seq).padStart(3, '0');
  // 메모리 어댑터는 'n19'(하루 자료), 서버는 접수 번호에서 만든 id('n261226019': 날짜가 달라도 겹치지 않는다).
  const id = idStyle === 'dated' ? 'n' + receiptNo.replace('-', '') : 'n' + seq;
  ...
  for (const s of plan.sections) {
    ...
    s.lines.forEach((q, i) => {
      ...
      lines.push({
        id: id + '-l' + n,
        kind: product.key,
        label: lineLabel(state.registry, q),
