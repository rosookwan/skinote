// 읽기 모델 만들기(ui 7절의 서버 몫): 화면 설정(칸 · 탭 · 숫자)을 읽어 줄마다 칸 값을 만든다.
...
function promiseParts(ctx: ViewContext, o: FxOrder): FitPart[] {
  ...
  const where = p.mode === 'store' ? '매장' : placeLabel(ctx.state.registry, p.placeId);
  return [
...
function placeCell(ctx: ViewContext, o: FxOrder): LedgerCell {
  const p = currentReturn(o);
  if (p.mode === 'store') return { renderer: 'place', parts: [{ text: '매장', drop: 0 }] };
  const reg = ctx.state.registry;
  return { renderer: 'place', parts: [{ text: placeLabel(reg, p.placeId), short: placeShortLabel(reg, p.placeId), drop: 0, words: true }] };
}

/** 차량 칸: 차량 배달 · 수거가 있는 팀만. */
function vehicleCell(ctx: ViewContext, o: FxOrder): LedgerCell | null {
  const id = currentReturn(o).vehicleId ?? o.pickup.vehicleId;
  return id ? { renderer: 'vehicle', parts: [{ text: vehicleLabel(ctx.state.registry, id), drop: 0 }] } : null;
}
...
function cellFor(ctx: ViewContext, o: FxOrder, column: LedgerColumnRow): LedgerCell | null {
  switch (column.renderer_key) {
    ...
    case 'place': return placeCell(ctx, o);
...
function nightPrep(ctx: ViewContext, orders: readonly FxOrder[]): LedgerViewResult['nightPrep'] {
  ...
  return top ? { slotAt: iso(slotAt), placeLabel: placeLabel(ctx.state.registry, top[0]), teams: top[1] } : undefined;
}
...
function listStamp(ctx: ViewContext, o: FxOrder, column: LedgerColumnRow, counter: boolean, pending: boolean): StampCell {
  ...
  if (counter && rule && DRIVER_RULES.has(rule) && (cell.state === 'todo' || cell.state === 'partial')) {
    return {
      ...cell,
      pressNote: {
        lines: [vehicleLabel(ctx.state.registry, o.giveBack.vehicleId) + ' 수거 예정'],
...
export function collectionList(ctx: ViewContext, params: ViewParams): LedgerViewResult {
  ...
  const vehicle = { id: vehicleId, label: vehicleLabel(ctx.state.registry, vehicleId) };
  return {
    ...head(ctx),
...
export function deliveryList(ctx: ViewContext, params: ViewParams): LedgerViewResult {
  ...
  const rows: LedgerRow[] = shown.map((t, i) => {
    ...
    for (const column of v.columns) {
      if (column.renderer_key === 'stamp') {
        const cell = driverColumnStamp(ctx.state.registry, o, column.step_keys ?? [], all, today);
  ...
  return {
    ...
    deviceClass,
    titleValues: { date, vehicle: vehicleLabel(ctx.state.registry, vehicleId) },
    activeTabKey: tabKey,
    ...
    primaryFigure: van.figure,
    nextTask,
    vehicle: { id: vehicleId, label: vehicleLabel(ctx.state.registry, vehicleId) },
    vehicleLoad: van.load,
    visitReasons: visitReasons(ctx.state.registry, 'deliver'),
  };
}
...
/**
 * 수납 수단 이름(매장 목록의 결제 수단). 보증금 결제는 반납 창의 `미수 차감`이 남긴 수납 쪽 기록이고(문구 표 3-9), 시스템 수단이라 매장
 * 목록에 없다.
 */
const DEPOSIT_METHOD_LABEL = '보증금 결제';
const methodLabel = (reg: { readonly payMethods: ShopRegistry['payMethods'] }, key: string) => (key === 'deposit' ? DEPOSIT_METHOD_LABEL : payMethodOf(reg, key)?.label ?? '');
  ...
  for (const column of columns) {
    switch (column.renderer_key) {
      ...
      case 'stamp': {
        ...
        if (!step) break;
        const day = bizDay(ctx.state);
        const cell = lineStamp(ctx.state.registry, o, l, step, all, day);
        ...
        else if (o.lines.some((x) => lineStamp(ctx.state.registry, o, x, step, all, day))) cells[column.column_key] = { renderer: 'stamp', stamp: { stepKey: step.key, state: 'na' } };
        break;
      }
      default:
        break;
...
export function orderSlip(ctx: ViewContext, orderId: string, deviceClass: DeviceClassKey = 'pos'): OrderSlip | null {
  ...
  const promiseLine = (kind: 'pickup' | 'return') => {
    ...
    return {
      ...
      parts: [
        { text: p.mode === 'store' ? '매장' : placeLabel(ctx.state.registry, p.placeId), drop: 1 },
        ...(p.mode === 'vehicle' ? [{ text: vehicleLabel(ctx.state.registry, p.vehicleId), drop: 2 }] : []),
        ...(p.note ? [{ text: p.note, drop: 3 }] : []),
...
function issueDeposit(ctx: ViewContext, o: FxOrder, picks: Pick[]): { summary: string; label: string; then: ConfirmStep[] } | null {
  ...
  return {
    summary: [due.rule.label, due.units + unit, '매장 기준 1' + unit + ' ' + won(due.unitAmount), '반납 시 반환', methodLabel(ctx.state.registry, due.methodKey) + ' ' + won(due.amount)].join(' · '),
...
function routeResetDraft(ctx: ViewContext, vehicleId: string): ConfirmDraftView {
  const title = ACTION_LABELS.route_reset + ' · ' + vehicleLabel(ctx.state.registry, vehicleId);
...
export function confirmDraft(ctx: ViewContext, params: ConfirmDraftParams): ConfirmDraftView {
  ...
  if (params.actionKey === 'print') {
    return notice(ctx, ACTION_LABELS.print, linesOf(ctx).unsupported);
  }
  ...
  switch (params.actionKey) {
    ...
    case 'pin': {
      ...
      return {
        ...
        title,
        summary: [
          [hm(at) + ' ' + placeLabel(ctx.state.registry, task.promise.placeId), o.teamName, o.last4, ...(note ? [note] : [])].join(' · '),
          vehicleLabel(ctx.state.registry, task.promise.vehicleId) + ' 수거 목록 맨 위 고정 · 기사 알림',
        ],
    ...
    case 'pay': {
      ...
      return {
        ...
        methods: ctx.state.registry.payMethods.filter((m) => m.quick).map((m) => ({ key: m.key, label: m.label })),
    ...
    default:
      return notice(ctx, label(params.actionKey), linesOf(ctx).unsupported);
  }
}
