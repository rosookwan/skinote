// V6 하루 마감(spec 3-7, ui 6-8, data-model 4-12 · 4-13 · S16 · S17): 마감 화면의 읽기 모델(closingSheet)과
...
// (입금 − 반환) + 확인한 차량 인계(센 금액)다. rev 창 대신 영업일로 센다(data-model 4-13의 rev 창은 저장소 몫).
...
const drawerLabel = (state: ShopState, id: string) => state.drawers.find((d) => d.id === id)?.label ?? '';

/** 차액 사유(reason_codes, 매장 목록 registry.cashReasons, spec 3-7 · 문구 표 3-9). 직접 입력(manual)은 글을 함께 적는다. */
const MANUAL = 'manual';
const isReason = (state: ShopState, key: string | undefined) => state.registry.cashReasons.some((r) => r.key === key);
...
const inDay = (state: ShopState, date: string) => (ms: number) => businessDateOf(ms, shopCutoff(state.settings)) === date;
 ...
 */
export function postingDate(state: ShopState, ms: number): string {
...
const postedOn = (state: ShopState, date: string) => (ms: number) => postingDate(state, ms) === date;
...
function depositCash(state: ShopState, date: string, drawerId: string): number {
  const day = postedOn(state, date);
...
const transfersOf = (state: ShopState, date: string, vehicleId: string) => state.cashTransfers.filter((t) => t.vehicleId === vehicleId && postedOn(state, date)(t.at));
 ...
 */
function counterExpected(state: ShopState, date: string): { amount: number; deposit: number } {
...
function lastReceipt(state: ShopState, date: string, vehicleId: string): number | undefined {
...
const reasonOk = (state: ShopState, diff: number, reasonKey: string | undefined, reasonNote: string | undefined) =>
  diff === 0 || (isReason(state, reasonKey) && (reasonKey !== MANUAL || (reasonNote ?? '').trim() !== ''));

function methodRows(state: ShopState, date: string): ClosingMethodRow[] {
  const day = postedOn(state, date);
  ...
  for (const o of state.orders) {
    for (const p of o.payments) {
      ...
      const vehicle = p.drawerId ? walletVehicle(state, p.drawerId) : undefined;
...
function vanRow(state: ShopState, date: string, item: VanItem, now: boolean): CashCheckRow {
  ...
  const base = { key: item.key, kind: 'van' as const, label: drawerLabel(state, vanDrawer(item.vehicleId)), note, expected: [{ text: '예상 ' + won(item.amount) }] };
  const c = t?.confirmed;
  if (c) {
...
function counterRow(state: ShopState, plan: ClosingPlan, now: boolean): CashCheckRow {
  const opening = '시재 ' + won(state.settings.openingCash);
  const base = { key: COUNTER, kind: 'drawer' as const, label: drawerLabel(state, COUNTER), note: [{ text: opening + ' 포함', drop: 1, short: opening }] };
...
function dueCarry(state: ShopState, now: number, date: string): CarryItem[] {
...
function returnCarry(state: ShopState, now: number, date: string): CarryItem[] {
  ...
  for (const section of state.registry.paySections) {
...
function depositCarry(state: ShopState, date: string): CarryItem[] {
  ...
  for (const d of held) {
    for (const { o, l } of lines) {
      ...
      section = section || (state.registry.paySections.find((s) => s.key === l.section)?.label ?? '');
    }
  }
  // 돈이 있는 곳: 현금 보증금은 돈통(차량 지갑에서 받았어도 그 차량이 인계했으면 돈통), 아직 인계 전이면 그 차량.
  const first = held[0]!;
  const take = first.entries.find((e) => e.kind === 'take');
  const vehicle = take?.drawerId ? walletVehicle(state, take.drawerId) : undefined;
  const handed = vehicle !== undefined && transfersOf(state, date, vehicle).some((t) => t.at >= take!.at);
  const where = take?.methodKey !== 'cash' ? undefined : vehicle && !handed ? vehicleLabel(state.registry, vehicle) + ' 보관' : '돈통 보관';
  return [{
    key: 'deposit',
...
function reviewCarry(state: ShopState, date: string): CarryItem[] {
...
function transferCarry(state: ShopState, date: string, plan: ClosingPlan): CarryItem[] {
  return plan.vans.filter((x) => x.deferred).map((x) => {
    ...
    return {
      ...
      note: [{ text: vehicleLabel(state.registry, x.vehicleId), drop: 0 }, { text: receipt !== undefined ? hm(receipt) + ' 입고' : '입고 대기', drop: 1 }], late: false,
    };
  });
}

function carryItems(state: ShopState, now: number, date: string, plan: ClosingPlan): CarryItem[] {
 ...
 */
export function closeConfirmLine(state: ShopState, now: number, date: string): string | undefined {
  const cutoff = shopCutoff(state.settings);
  const facts: string[] = [];
  ...
  for (const v of state.registry.vehicles) {
    ...
    if (out) facts.push(vehicleLabel(state.registry, v.id) + ' 미입고');
  }
...
function afterMidnight(state: ShopState, date: string): string | undefined {
  const day = inDay(state, date);
  const first = new Map<FxOrder, number>();
  for (const o of state.orders) {
    for (const l of o.lines) {
...
function reasonOptions(state: ShopState, diff: number | undefined, reasonKey: string | undefined): ChoiceOption[] {
  const open = diff !== undefined && diff !== 0;
  return state.registry.cashReasons.map((r) => ({ key: r.key, label: r.label, selected: open && r.key === reasonKey, enabled: open, ...(r.key === MANUAL ? { opens: true } : {}) }));
}

/** 열린 점검 판(차량 현금 · 돈통): 친 금액의 차액 · 사유 · 판의 주 버튼 · 보낼 것. 그 줄이 점검할 차례가 아니면 없음. */
function checkView(state: ShopState, plan: ClosingPlan, input: NonNullable<ClosingSheetParams['check']>): ClosingCheckView | undefined {
  ...
  const ok = diff !== undefined && reasonOk(state, diff, reasonKey, input.reasonNote);
  const reason = reasonKey && isReason(state, reasonKey) ? { reasonKey, ...(reasonKey === MANUAL ? { reasonNote: (input.reasonNote ?? '').trim() } : {}) } : {};
  ...
  const view: ClosingCheckView = {
    key: input.key,
    kind: van ? 'van' : 'drawer',
    title: van ? drawerLabel(state, vanDrawer(van.vehicleId)) : drawerLabel(state, COUNTER),
    expected: van ? [{ text: '예상 ' + won(expected) }] : expectedRuns(plan.counter),
    diff: diffLine,
    reasons: reasonOptions(state, diff, reasonKey),
...
function frozenSheet(state: ShopState, now: number, plan: ClosingPlan): FxClosingSheet {
...
export function closingSheet(ctx: ViewContext, params: ClosingSheetParams): ClosingSheetView {
  ...
  if (date !== state.businessDate) throw new DomainError('NOT_FOUND', '없는 마감: ' + date);
  ...
  const check = params.check ? checkView(state, plan, params.check) : undefined;
...
const closedOn = (state: ShopState, date: string) => state.closings.some((c) => c.date === date);

/**
 * 차량 현금 인계(cash.transfer, 기사 → 넘기는 중 돈통): 매장 입고 때 기사가 봉투를 건넨다(이야기 23:48). 연결이 필요하다. 그 영업일을
 * 마감한 뒤의 인계는 다음 열린 날의 것이다(postingDate).
 */
export function transferCash(state: ShopState, envelope: CommandEnvelope<'cash.transfer'>, now: number, lines: DomainLines): Result {
  const p = envelope.payload;
  const amount = Math.floor(p.amount);
  if (!state.registry.vehicles.some((v) => v.id === p.vehicleId) || !(amount > 0)) return unsupported(lines);
 ...
 */
export function confirmTransfer(state: ShopState, envelope: CommandEnvelope<'cash.transfer_confirm'>, now: number, lines: DomainLines): Result {
  const p = envelope.payload;
  ...
  const vehicleId = found ? found.vehicleId : walletVehicle(state, p.transferId);
  if (!vehicleId) return unsupported(lines);
  ...
  if (!(counted >= 0)) return unsupported(lines);
  if (!reasonOk(state, counted - expected, p.reasonKey, p.reasonNote)) return rejected('차액 ' + diffWon(counted - expected) + ' · 사유 선택');
 ...
 * 마감 몫이라 아직 없다(열린 날 여럿의 마감은 B2d, work/impl-server/plan.md §3-3 14).
 */
export function closeDay(state: ShopState, envelope: CommandEnvelope<'closing.close'>, now: number, lines: DomainLines): Result {
  const p = envelope.payload;
  if (p.date !== state.businessDate) return unsupported(lines);
