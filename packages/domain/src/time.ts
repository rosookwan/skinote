// 시각 도우미. 매장 시간대는 한국(UTC+9, 서머타임 없음)으로 고정한다(D13: 서버는 Asia/Seoul이 아닌 매장을 읽지 않는다). 영업일은
// 'YYYY-MM-DD'.
...
/** 분 단위로 내린 시각(약속 시각은 'HH:MM'으로 적는다, plan §3-3 6). */
export const floorMinute = (ms: number): number => Math.floor(ms / MINUTE) * MINUTE;
 ...
 * 그때의 기준으로 센다(data-model 3-3 · ADR-06: 저장한 영업일은 다시 계산하지 않는다 — 이 상태는 영업일을 저장하지 않으므로 기록
...
/** 12시간(세션이 끝나는 영업일 기준 시각까지 적어도 이만큼). */
const SESSION_MIN = 12 * HOUR;

/**
 * 카운터 로그인이 끝나는 때(plan §5-3): 로그인에서 적어도 12시간 뒤인 첫 영업일 기준 시각(06:00). 07:00 로그인은 다음 날 06:00,
 * 05:50 로그인은 그날 06:00이 10분 뒤라 다음 날 06:00. 밤 반납까지 같은 로그인으로 일하고 가게가 닫힌 새벽에 끝난다.
 */
export function sessionExpiry(loginAt: number, cutoff: string): number {
  const [h = 0, m = 0] = cutoff.split(':').map(Number);
  const day = kstDate(loginAt);
  for (let offset = 0; ; offset += 1) {
    const at = kstAt(day, offset, h, m);
    if (at - loginAt >= SESSION_MIN) return at;
  }
}
