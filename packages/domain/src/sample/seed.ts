// 견본 하루(sampleDay): 시안(docs/design/redesign-mockups.html)과 같은 하루 — 12월 26일 (토) 오후 3시 40분, 18팀. 날짜를 받아 그 날의
// 하루로 만든다(시각 · 접수 번호의 날짜 부분이 따라간다). 체험판은 12월 26일로 부른다(apps/pos/src/fixture/demo.ts).
...
function makeOrder(input: OrderInput): FxOrder {
  const lines = input.lines(input.id);
  const charged = lines.reduce((sum, l) => sum + l.amount, 0);
  return {
    id: input.id,
    receiptNo: input.prefix + '-' + String(input.no).padStart(3, '0'),
