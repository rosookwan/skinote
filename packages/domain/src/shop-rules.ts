...
// 카드는 규칙 행에서 나온다(ADR-05): 반납 정책을 매장이 고르는 품목 종류마다 `○○ 반납`
// (지금은 리프트권 하나), 보증금 규칙마다 `○○ 보증금`(지금은 리프트권 보증금 하나), 그리고 매장 설정 셋(리프트권 결제 · 당일 취소
// 환불 · 영업일 기준 시각). 화면은 누른 버튼의 key를 바꿈(RuleChange)으로 모아 다시 묻고, 여기서 카드 · 바뀐 곳(전 → 후) · 바닥줄 ·
// 주 버튼 · 보낼 명령 · 거절 까닭을 새로 쓴다. 저장은 다음 기록부터다: 만든 줄 · 맡은 보증금은 만들 때 복사한 값을 쓰고(catalog
...
import type { DomainLines, FxDepositRule, FxShopRules, ShopRegistry, ShopState } from './model.ts';
...
type Reg = Pick<ShopRegistry, 'kinds' | 'products'>;

/**
 * 리프트권 보증금 규칙의 시작 값: 보증금 규칙을 한 번도 둔 적 없는 매장이 `사용`을 누르면 이 값으로 시작한다(spec 3-9 · 2-3, 1매 5,000원 현금 ·
 * 접수 시 · 미반납 시 몰수 · 분실 35,000원 · 반납 다음 날 정리).
 */
export const START_LIFT_DEPOSIT: Readonly<FxDepositRule> = Object.freeze({
  key: 'lift_ticket_card', label: '리프트권 보증금', section: 'lift', unitAmount: 5_000, timing: 'at_intake', refundDefault: 'cash',
  unreturned: 'keep', lossAmount: 35_000, afterDays: 1, methods: Object.freeze(['cash']) as FxDepositRule['methods'],
} satisfies FxDepositRule);
...
/** 금액 숫자판이 받는 범위. 영업일 기준 시각은 00:00 ~ 11:59(spec 3-9). */
...
/** 반납 정책을 매장이 고르는 품목 종류(지금은 리프트권 칸의 종류). 장비는 늘 반납이라 카드가 없다. */
const policyKinds = (reg: Reg) => reg.kinds.filter((k) => k.products.some((p) => productOf(reg, p)?.section === 'lift'));

/** 이 매장의 보증금 규칙(미사용으로 바꾼 동안에는 둔 값, 없으면 시작 값). */
const depositRule = (settings: FxShopRules): FxDepositRule =>
  settings.liftDeposit ?? settings.liftDepositOff ?? { ...START_LIFT_DEPOSIT, methods: [...START_LIFT_DEPOSIT.methods] };

/** 카드 · 줄의 바꿈 key(RuleChange.key, '<표>:<행>:<열>'). */
export function ruleKeys(reg: Reg, settings: FxShopRules) {
  const rule = depositRule(settings).key;
  const kind = policyKinds(reg)[0]?.key ?? 'lift';
...
export function draftValues(reg: Reg, settings: FxShopRules, changes: readonly RuleChange[] = []): { values: RuleValues; invalid: RuleChange[] } {
  const keys = ruleKeys(reg, settings);
...
export function diffValues(reg: Reg, settings: FxShopRules, after: RuleValues): Diff[] {
  const keys = ruleKeys(reg, settings);
  const before = ruleValues(settings);
  const unit = unitOf(reg);
  const kind = policyKinds(reg)[0]?.label ?? '리프트권';
...
/** 리프트권의 단위(`매`). */
const unitOf = (reg: Reg) => productOf(reg, policyKinds(reg)[0]?.products[0])?.unit ?? '매';
...
function cards(state: ShopState, v: RuleValues, changed: ReadonlySet<string>): RuleCard[] {
  const keys = ruleKeys(state.registry, state.settings);
  const unit = unitOf(state.registry);
  const rule = depositRule(state.settings);
  const kinds = policyKinds(state.registry);
...
export function shopRules(ctx: ViewContext, params: ShopRulesParams): ShopRulesView {
  const { state, now } = ctx;
  const { values } = draftValues(state.registry, state.settings, params.changes ?? []);
  const diff = diffValues(state.registry, state.settings, values);
  const n = diff.length;
 ...
 * 그 값으로 다시 시작한다.
 */
export function applySettings(state: ShopState, envelope: CommandEnvelope<'setting.set'>, now: number, lines: DomainLines): Result {
  const changes = envelope.payload.changes ?? [];
  const { values, invalid } = draftValues(state.registry, state.settings, changes);
  if (invalid.length) return unsupported(lines);
  const diff = diffValues(state.registry, state.settings, values);
