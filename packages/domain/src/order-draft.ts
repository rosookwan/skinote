// V2 새 접수 ① 품목 · V3 ② 일정(spec 3-4 · 3-5, ui 6-6)의 읽기 모델(orderDraft).
...
//   - 종류 타일 · 규격 · 권종 · 값은 매장 목록(registry, catalog.ts의 셈)에서. 보증금은 합계에 넣지 않고 `보증금 · 별도`로 따로 적는다.
...
export function resolveSchedule(state: ShopState, now: number, draft: OrderDraftInput): { pickup: ResolvedPickup; giveBack: ResolvedReturn; quote: Quote } {
  ...
  const quote = quoteOf(state.registry, draft.items, state.settings, dayCount(pickup.date, date));
...
const dayOfDate = (state: ShopState, date: string) => dayWord(kstAt(date, 0, 12, 0), state.businessDate, shopCutoff(state.settings));
/** '오늘 22:00' · '내일 09:00' · '오늘 24:00'(심야). */
const dayTime = (state: ShopState, at: number) => dayWord(at, state.businessDate, shopCutoff(state.settings)) + ' ' + timeText(state, at);
...
function tileOf(reg: ShopRegistry, kind: FxKind, quote: Quote, openKey: string | undefined): KindTile {
  ...
  if (kind.placement === 'grouped') {
    const chosen = kind.products.map((key) => reg.products[key]!).filter((p) => lines.some((l) => l.product.key === p.key));
    ...
    const product = reg.products[kind.products[0] ?? '']!;
...
function pickerOf(reg: ShopRegistry, kind: FxKind, quote: Quote, openVariant: string | undefined): OrderPicker {
  ...
  if (kind.placement === 'grouped') {
    const products = kind.products.map((key) => reg.products[key]!);
  ...
  const moreLabel = moreLabelOf(kind);
  const variants = { label: kind.label + ' ' + kind.axis, options, ...(moreLabel ? { moreLabel } : {}) };
  if (!open) return { kindKey: kind.key, variants, hint: kind.axis + ' 선택' };
  return {
    kindKey: kind.key, variants,
    quantity: {
...
function selectedRows(state: ShopState, quote: Quote): DraftLineView[] {
  ...
  for (const kind of state.registry.kinds) {
    const lines = quote.lines.filter((l) => l.product.kindKey === kind.key);
...
export function orderDraft(ctx: ViewContext, params: OrderDraftParams): OrderDraftView {
  ...
  // ① 품목
  const tiles = state.registry.kinds.map((k) => tileOf(state.registry, k, quote, openKind?.key));
  const picker = openKind ? pickerOf(state.registry, openKind, quote, params.openVariantKey) : undefined;
  ...
  const reserveLine = pickup.kind === 'reserve'
    ? [dayOfDate(state, pickup.date), pickupTime, pickup.mode === 'vehicle' ? (pickup.placeKey ? placeLabel(state.registry, pickup.placeKey) : '차량 배달') : STORE].filter(Boolean).join(' ')
    : undefined;
  const deliverLine = pickup.kind === 'deliver' ? [pickup.immediate ? '즉시' : pickupTime, pickup.placeKey ? placeLabel(state.registry, pickup.placeKey) : ''].filter(Boolean).join(' ') : undefined;
  ...
  const pickupNote = pickup.mode === 'vehicle' ? [pickup.placeKey ? placeLabel(state.registry, pickup.placeKey) : '', '차량 배달'].filter(Boolean).join(' · ') : STORE;
  const returnTitle = giveBack.available && giveBack.at !== undefined ? '반납 · ' + dayTime(state, giveBack.at) : '반납 · ' + dayOfDate(state, giveBack.date);
  const returnNote = giveBack.mode === 'vehicle' ? [giveBack.placeKey ? placeLabel(state.registry, giveBack.placeKey) : '', '차량 수거'].filter(Boolean).join(' · ') : STORE + ' 반납';
  const gearItems = [
    ...state.registry.kinds.filter((k) => k.placement === 'tile').flatMap((k) => {
