// 기사 기기의 배달 · 업무 판(V7, spec 3-8 · ui 6-5)의 셈: 배달 업무(차량 배달 접수 하나 = 업무 하나, 'deliver:<접수>'),
...
// 돈의 바탕(expect)은 참고 값이라 달라도 돌려보내지 않는다(8-12). 화면은 이 셈을 모른다(읽기 모델만).
...
export function findDeliverTask(state: ShopState, taskId: string): FxDeliverTask | undefined {
...
export function resolveTask(state: ShopState, taskId: string): FxAnyTask | undefined {
...
export function fieldDue(_state: ShopState, o: FxOrder): number {
  return selfDue(o);
}
...
function lastPayment(state: ShopState, o: FxOrder): string | undefined {
  ...
  return at + ' ' + (payMethodOf(state.registry, last.methodKey)?.label ?? '') + ' 수납';
}
...
function returnPlan(state: ShopState, o: FxOrder): string[] {
...
export function taskSheet(ctx: ViewContext, params: TaskSheetParams): TaskSheetView {
  ...
  const place = t.promise.mode === 'store' ? '매장' : placeLabel(state.registry, t.promise.placeId);
  ...
  return {
    ...
    progress,
    primary,
    visitReasons: visitReasons(state.registry, t.kind),
    vehicle: { id: vehicleId, label: vehicleLabel(state.registry, vehicleId) },
  };
}
...
export function ticketQuote(state: ShopState, o: FxOrder, productKey: string, quantity: number): { amount: number; deposit?: { ruleKey: string; amount: number; unitAmount: number }; hash: string } {
  const product = productOf(state.registry, productKey);
...
export function addTicketSheet(ctx: ViewContext, params: AddTicketSheetParams): AddTicketSheetView {
  ...
  if (quote.deposit && rule) {
    ...
    lines.push([
      ...
      { text: ' · 반납 시 반환' },
      { text: ' · ' + (payMethodOf(state.registry, rule.methods[0] ?? 'cash')?.label ?? '') + ' ' + won(quote.deposit.amount), strong: true },
    ]);
  }
 ...
 */
export function collectDepositStep(state: ShopState, task: FxTask, picks: readonly { l: FxLine; qty: number }[]): { summary: string; label: string; then: ConfirmStep[] } | null {
  const rule = heldRule(state);
  const o = task.order;
  const dep = rule ? depositOf(state, o.id, rule.key) : undefined;
  if (!dep) return null;
...
function taskOf(state: ShopState, taskId: string, orderId: string): FxAnyTask | undefined {
 ...
 */
export function fieldCollect(state: ShopState, envelope: CommandEnvelope<'field.collect'>, now: number): Result {
...
export function ticketTaken(state: ShopState, o: FxOrder, requestId: string): { label: string; lineAmount: number; deposit: number } | null {
  ...
  return { label: state.registry.paySections.find((x) => x.key === line?.section)?.label ?? line?.shortLabel ?? '', lineAmount: line?.amount ?? 0, deposit: entry.amount };
}
 ...
 */
export function fieldDepositReturn(state: ShopState, envelope: CommandEnvelope<'field.deposit_return'>, now: number): Result {
  const p = envelope.payload;
  const t = taskOf(state, p.taskId, p.orderId);
  if (!t) return rejected('업무 없음');
 ...
 * 기사 기기는 이 팀 자신(payerOrderId null · 이 팀)만: 다른 팀을 결제 팀으로 정하는 것은 카운터의 결정이다(미지원 입력).
 */
export function setPaymentPromise(state: ShopState, envelope: CommandEnvelope<'payment_promise.set'>, _now: number, lines: DomainLines): Result {
  const p = envelope.payload;
  const o = findOrder(state, p.orderId);
  if (!o) return rejected('접수 없음');
  if (p.payerOrderId !== null && p.payerOrderId !== o.id) return unsupported(lines);
