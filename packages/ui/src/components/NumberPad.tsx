...
import { formatPhone, formatTimeDigits, formatWon } from '../format.ts';
import { t } from '../strings.ko-KR.ts';
import { TextFit } from './TextFit.tsx';

export type NumberPadMode = 'phone' | 'amount' | 'time';
...
/** 모드마다 칠 수 있는 숫자 수(휴대폰 11, 시각 HHMM 4, 금액 9자리 = 999,999,999원). */
export const NUMBER_PAD_DIGITS: Readonly<Record<NumberPadMode, number>> = { phone: 11, amount: 9, time: 4 };

/** 표시 칸의 글: 연락처 '010-0000-0042', 시각 '17:2_', 금액 '160,000원'(빈 금액은 비움). */
...
/** `입력`을 누를 수 있는지: 시각은 네 자리일 때만(그 밖은 빈 값도 넘긴다 — 빈 연락처는 지움). */
export const numberPadReady = (mode: NumberPadMode, digits: string) => mode !== 'time' || digits.length === NUMBER_PAD_DIGITS.time;
    ...
    >
      {keysOnly ? null : (
        <>
          ...
          <output className={'sn-keypad-display sn-numpad-display' + (digits.length ? ' has-value' : '')} aria-live="polite">{text}</output>
