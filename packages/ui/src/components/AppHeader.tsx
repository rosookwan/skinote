...
import type { MenuEntryRow } from '@skinote/contract';
...
export function AppHeader({ shopName, vehicleLabel, menu, alertCount, currentKey, onHome, onFind, onMenu, onMore, onAlerts, onExit }: AppHeaderProps) {
  ...
  useIsoLayoutEffect(() => {
    ...
    setLevel(0);
  }, [bar?.width, profile.capacity.menu, fonts, menuSignature, Boolean(onFind), Boolean(onHome), vehicleLabel]);
  ...
  useIsoLayoutEffect(() => {
    ...
    const fixed = [...left.children].filter((el) => !el.classList.contains('sn-header-shop'));
    const leftNeed = fixed.reduce((w, el) => w + (el as HTMLElement).offsetWidth, 0) + (parseFloat(ls.columnGap) || 0) * fixed.length;
  ...
  return (
    <header ref={barRef} className="sn-header">
      <div ref={leftRef} className="sn-header-left">
        ...
        <TextFit className="sn-header-shop" input={{ mode: 'parts', parts: [{ text: shopName, drop: 1 }] }} />
