...
| `apps/pos` | React 앱(포스 + 기사), HttpClient · LocalClient(체험판, 기사 오프라인, **카운터 오프라인**), 연결 띠 · 보냄 대기 · 임시 접수 번호 | |
...
| 지급 · 매장 반납 · 적재 · 전달 · 받음 · 매장 입고 · 발권 · 권 회수 · 발권처 환불 시도 · 교환 완료 · 재고 들이기 · 정비 상태 · 찾음 · 사이즈 입력 · 거래처 거래와 정산 | **adapter**(native 사실 어댑터 + 옛 처리기) | 검증된 옛 이동 규칙을 쓰되 사실은 거절하지 않음. 옛 이동 종류에 적재 · 전달 · 받음 · 입고가 이미 있다. 오프라인 허용 이동은 앞 단계가 행을 만들고 옛 처리기는 대조(3-2) |
| 끊긴 카운터의 권 주기(`ticket.hand_out`: 배정 + 지급) · 바꿔 드림(`exchange.swap`: 같은 줄에서 번호를 바꿈) | native(옛 모양으로 표현되는 모양: 권 배정 claim 하나와 지급 이동, 교환 하나 · 교환 장비 하나와 반납 · 지급 이동) | 끊긴 동안의 사실이라 facade 뒤에 둘 수 없다(sync 8-2). 표현 규칙(3-4)을 지키므로 옛 처리기가 뒤에 그 묶음을 읽을 수 있다 |
...
| 이름 | 언제 | 무엇 |
|---|---|---|
| **F1 엔진 대안** | G1(11/6)을 넘지 못하면 | 2026-27 시즌을 ski-rent-ops 서버 + 포스 v4로 운영하고, N1(적재 버튼) · N2(반납 장소 줄) · N12(순서 바꾸기 · 빨리 확인) · N19(여러 팀 결제를 '같은 메모의 한 팀씩 결제'로)를 옛 코드에 덧붙인다. 옛 서버(`returns-api.cjs`)는 **한 번도 인터넷에 올린 적이 없는 로컬 전용 서버**(정적 토큰 파일 로그인, 기본은 로컬 주소만 수신)라, 우리 중앙 서버(한국)에 올리는 운영 준비를 **4 ~ 5주차에 미리 해 둔다**(6절: 연습 서버에 올림, TLS · 허용 출처 · 요청 제한 · 계정 파일, 운영 설치 스크립트 재사용, 복제 · 날마다 스냅샷 · 바깥 가동 확인, 가짜 자료로 하루 연습). 옛 앱에는 카운터 오프라인이 없고 보냄 대기가 한 칸이라 가게 인터넷이 20분 끊기면 수납 · 반납 도장도 찍지 못한다: **F1 운영 조건**은 예비 회선 자동 넘어가기(필수), 끊긴 동안의 종이 수납 · 반납 양식과 연결 뒤 입력 절차, 아침 종이 인쇄(4-1)다. G1에서 F1을 고르면 운영 준비는 이미 되어 있어 남은 4주는 덧붙이기와 연습에 쓴다. 새 시스템은 시즌 내내 그림자로 돌리며 대조하고, 시즌이 끝난 뒤 한 시즌 자료와 함께 옮긴다. 설계는 바뀌지 않고 날짜만 옮긴다 |
...
되돌리기(개장 전까지): 11월 바꾸기 때 옛 파일은 읽기 전용으로 옆에 둔다(중앙 서버 안). 되돌려야 하면 F1 준비로 중앙에 올려 둔 옛 서버(4 ~ 5주차)를 옛 파일로 다시 켜고, 연습 기간에 기록한 명령(모드 E 기록)을 옛 엔진에 다시 넣는다. native만의 명령은 옛 모양으로 낮춘다: 여러 팀 수납 → 접수별 결제 여러 개(같은 메모), 빨리 확인 → 우선 요청 알림, 방문 순위 → `sequences`, 선입금 → 메모 '선입금'인 결제, 결제 약속 → 메모. 시즌이 열린 뒤에는 옛 파일로 되돌리지 않는다. 고칠 것은 앞으로 고친다.

### 4-1. 개장일에 가게 인터넷이나 우리 서버가 멈추면

- **가게 인터넷이 끊김:** 예비 회선 자동 넘어가기 공유기가 몇 초 안에 LTE로 넘어간다(개장 조건). 넘어가지 못하면 카운터는 연결 띠를 보이며 지급 · 반납 · 적재 · 권 주기 · 수납 · 보증금 · 새 현장 접수(임시 접수 번호) · 인쇄를 계속한다(sync 8-2 · 8-6). 기사 기기는 LTE로 그대로 붙어 있다. 약속 바꾸기 · 취소 · 할인 · 마감은 연결된 뒤에 하고, 그사이 할 결정은 종이 양식 `연결된 뒤 할 일`에 적는다(deployment 6-5). 개발자는 기기 목록(관리자 콘솔 · 명령줄 도구)으로 가게가 끊긴 것을 본다.
...
| 4 (10/19–10/23) | `/api/v2/sync`(커서 rev.seq, 기기 판에 맞춰 모르는 열을 뺌) + SSE(하트비트, 끊기면 묻기), 표마다 범위 함수 + 범위 속성 시험(카운터 사본 포함), 범위가 바뀐 기기 reset, 기사 기기 LocalClient(OPFS 범위 자료 + 보냄 대기 겹치기), 카운터 부트스트랩의 '오늘 일할 사본' 고르기(sync 8-1). **운영:** 연습 서버 + 운영 VM 한 대(볼륨 암호화, 예약 IP) + TLS(DNS-01), `deploy/` 설치 스크립트와 GitHub Actions(환경 보호: 연습 자동 · 운영 사람이 누름), 바깥 가동 확인과 전화 알림(깨움 · 아침 묶음), **F1 준비 시작**(옛 서버를 연습 서버에 올림: TLS · 허용 출처 · 요청 제한 · 계정 파일). **가게: 예비 회선 자동 넘어가기 공유기 설치 · 선 뽑아 확인** | **N12** 방문 순서 ▲▼ · 맨 위로 · 시간순(기사 · 카운터, 기준 업무 + 나중 결정이 이김), 빨리 확인(고정 + 세 단계 알림), **N3** 수거 목록 읽기 모델 + A4 인쇄(같은 화면 설정의 인쇄 판 · outbox · 인쇄 도우미) | C3 기사 수거 목록(태블릿 · 휴대폰), 숫자판(옆 판 · 아래 판), RowActionBar, PinBar, 연결 띠, 포스 수거 목록 + 인쇄 | 두 기기가 동시에 순서를 바꿔도 충돌 없음, 수거 목록이 인쇄됨, 온라인 · 오프라인 `driverList`가 같음, 연습 · 운영 서버가 TLS로 뜸 |
| 5 (10/26–10/30) | 옛 `finance.*` 번역기, 의도한 차이 목록, 모드 D(돈 · 마감). **운영:** WAL 복제(다른 구역 + 다른 회사) · 올리지 않은 커밋 경보 · 날마다 스냅샷 · 1분 구간(두 부분) · **손으로 새 VM에 한 번 되살리기**(절차서 첫 판), F1 준비 마무리(복제 · 가동 확인을 붙이고 가짜 자료로 하루) | 돈 native: 수납 · 환불 · **N19 한 번에 수납**(수납 하나 + 배분) · 보증금((접수, 규칙)으로 보관 찾기) · 조정 · 현금 · 차량 지갑 · 넘기기, **마감**(돈통별, `business_days`, `posting_date`, 보낼 기록이 남은 모든 기기가 막음) | 수납 창, 한 번에 수납 창, 마감 화면 | 여섯 팀 한 번 수납에 모든 접수 미수 0, 마감이 옛 엔진 대조와 같음 |
| 6 (11/2–11/6) | 오프라인 큐 · batch · dependsOn(큐 명령은 멈추지 않음) · 빈 기기 번호 찾기 · 확인 필요 보일 곳과 `끊긴 동안 겹친 것`, 기기 서명 행위자 + `device_sign_ins`, **기기 종류마다의 오프라인 목록**(`sys_offline_commands`), **큐 명령은 적어 두는 사실**(sync 8-12)과 다시 붙는 창 · 다시 판단(8-3), **카운터 오프라인 사실**(LocalClient로 넘어가기 · 넘기기 지연 · 결과 확인 중 · 오늘 일할 사본 · 지급 · 반납 · 적재 · 권 주기 · 수납 · 보증금 · 두 카운터 겹침 · 끊긴 동안의 PIN · 도우미의 큐 한 벌), 기기 막기(보류)와 사본 지우기, 어댑터 앞 단계와 옛 처리기 견주기 시험, S5 · S6 · **S18(넓힌 것)** 시나리오 시험 자동화(오프라인 중 앱 다시 시작 포함) | 계정 · 직원 · 기기 등록(두 단계, `persist()`, 기기 번호) · 세션(기기 키에 묶인 쿠키) · PIN, **N20** 현장 수납 · 차량 예비권 추가(오프라인 견적), **N11** 방문 결과 버튼, 카드 결제 잠금 | 확인 필요(목록 · 띠 · 창 안 단계 · 끊긴 동안 겹친 것), 오프라인 표시(보냄 대기, **카운터 연결 띠와 연결을 기다리는 버튼의 한 문장**, 끊긴 반납 창), 로그인 · PIN 전환, 기사 업무 판 · 현장 수납 판 · 리프트권 추가 판 | **G1**(아래) |
...
**반드시 할 것(주가 밀려도) — 개장 필수:** 스키마와 실행기, 쓰기 순서와 검증기, codec과 facade(옛 기능 전부가 새 저장소에서 돎), 가져오기와 대조, 동기화. **운영:** 운영 VM 하나, 다른 구역으로 WAL 복제(올리기 전 암호화), 날마다 스냅샷, 다른 회사 저장소에 작업 기록 구간, 바깥 가동 확인과 전화 알림, 절차서대로 새 VM에 끝까지 되살리기 두 번(7 · 10주차), 가게의 예비 회선 자동 넘어가기, F1 준비(4 ~ 5주차). **카운터 오프라인 사실**(지급 · 반납 · 적재 · 권 주기 · 수납 · 보증금)과 큐 규칙(sync 8-12) · 마감이 모든 기기를 기다림. N1 · N2 · N12 · N3 · N4 · N19.

**밀리는 순서**(앞의 것부터 뒤로 간다): N20 → N17 → N11 → 화면 다듬기 → 가게 보관용 내보내기(손님 정보 포함) → 바꿔 드림(대신 교환 버튼의 한 문장과 접수증에 적기) → 카운터 오프라인 새 접수 · 품목 추가(대신 종이 현장 접수표를 쓰고 연결 뒤 입력; 임시 접수 번호도 함께 밀림).
...
**만들었지만 꺼 둠 · 나중:** 강습 명령 · 화면(기능 꺼짐), 나눠서 결제 화면(표는 있음, 카드 단말과 함께), 카드 단말 도우미와 실제 문자(outbox는 있음, 모의 대상), 요금 조건 편집 화면(첫 판은 옛 단가 그대로), 가게 LAN 중계(deployment 1-2), 혼합(시즌 뒤), PostgreSQL.
...
| 가게 인터넷 끊김(새 단일 고장점) | 예비 회선 자동 넘어가기(개장 조건, 4주차 설치), 카운터 오프라인(지급 · 반납 · 적재 · 권 주기 · 수납 · 보증금 · 새 현장 접수), 끊긴 동안 결정을 적는 종이 양식, 1주차부터 측정. 혼합은 시즌 뒤 |
