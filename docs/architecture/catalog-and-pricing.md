...
| `return_policy_key` | required · optional · none | **반납이 필요한가.** required: 돌아와야 줄이 끝남. optional: 지급으로 끝, 회수하면 기록하고 반납 도장. none: 반납 칸이 `—`. 리프트권은 **매장이 고른다**(가게 규칙 `리프트권 반납`: `꼭 돌려받음` = required · `주면 끝 · 돌아오면 적음` = optional; 둘째 판 시안 V8의 말). 이 매장은 required(2026-09-24) | 줄 완료, 반납 도장, 수거 목록 |
...
| lift_deposit | 리프트권 보증금(기능 deposits; 보증금 규칙의 칸, 할인 없음, `나중에` 없음 — 받지 못한 팀은 지급 창이 먼저 물음. 이 매장은 수단이 현금 하나라 확정 창에서 버튼 없는 한 줄, 15절) | 현금만 · 접수 때 | — |
...
**보증금 규칙**(`deposit_rules`, 11-1): `lift_ticket_card` · '리프트권 보증금' · 대상 종류 `lift_ticket` · **1매 5,000원(이 매장 값)** · 받는 때 `at_intake`(화면 `접수할 때 / 못 받으면 권을 줄 때`: 전화 예약처럼 그때 못 받으면 지급 창이 먼저 물음) · 결제 칸 `lift_deposit` · 돌려드리는 법 `cash`(현금; 미수가 있으면 반납 창의 `돌려드리는 방법`에서 `미수에서 빼기`를 고를 수 있음) · 안 돌아오면 `keep`(화면 `보증금 안 돌려드림`) · `unreturned_after_days` 1(반납 약속일 다음 영업일의 마감이 정리; 사장님 확인 전 시작 값, README 열린 질문 17). 기능 `deposits` 켬.

**매장 값**: 하루 기준 시각(가게 규칙 화면 `전날 장부로 치는 시각` · `새벽 6시`) `shops.business_day_cutoff = '06:00'`(스키 매장 기본값; data-model 3-3), 선입금 방식 `full_lift_ticket`, 당일 취소 환불 `refund`, 기사에게 미수 보이기 켬. 모두 관리 → 매장 설정 → 가게 규칙에서 바꾸고, 바꾸면 그다음 기록부터다.
...
**도장 단계**: `issue` 지급 · `return` 반납 · `pay` 수납 · `load` 적재 · `ticket_secure` 발권 · `collect` 받음(시각 표시) · `receive` 매장 입고 · `lesson_done` 강습(기능 lessons). 단계마다 동작(`sys_actions`: 명령 + 확인 창 + 되돌리기)이 하나다. 리프트권 종류는 이 매장에서 `return` 단계도 가진다(`item_kind_stamp_steps` 행 하나). 반납 정책이 optional인 매장은 회수한 권에만 반납 도장이 찍히고 회수 전에는 반납 칸이 `—`다.
...
- **오프라인 기록의 가격**(차량의 권 추가, 그리고 2026-09-24 ADR-19부터 끊긴 카운터의 새 현장 접수 · 품목 추가 · 권 주기): 기기가 가진 판으로 계산해 판 id · 금액 · `quote_hash`를 함께 보낸다. 서버는 그 판이 `occurred_at`에 효력이 있던 게시 판이면 그대로 받고(`price_source_key 'offline_quote'`), 아니면 기기 값으로 적고 `확인 필요`에 올린다. 권 추가는 사실(fact)이고, 새 접수 · 품목 추가는 등급이 intent여도 **큐에서 온 명령은 적어 두는 사실**로 다룬다(sync 8-12). 그래서 어느 쪽도 `QUOTE_CHANGED`로 돌려보낼 수 없고, 조용히 다시 계산하지도 않는다(현장에서 받은 돈과 청구가 어긋나지 않게). 온라인으로 바로 보낸 새 접수 · 품목 추가만 `QUOTE_CHANGED`를 받는다.
...
| `orders` 투영 | 청구 225,000 · 받음 105,000 · 미수 120,000 · 보증금 15,000(`deposit_held_amount`) · 수납 상태 partial. 카드 문구 '예약금 105,000원 수납 · 잔액 120,000원', 접수증 돈 줄 끝에 `보증금 15,000원` |
...
- **반납 정책은 매장이 고른다**(사장님 답 2026-09-24: 이 매장은 꼭 돌려받는다 — 리프트권 보증금이 있어서. 다른 스키장 · 매장은 그렇지 않을 수 있다). 고르는 곳은 가게 규칙 `리프트권 반납` 하나(종류의 `return_policy_key`)이고, 줄은 만들 때 정책을 복사하므로 이미 받은 접수는 바뀌지 않는다.
...
사장님 답(2026-09-24): 리프트권은 꼭 돌려받는다. 리프트권 보증금이 있어서다. 다른 스키장은 그렇지 않을 수 있으니 매장이 고르게 한다. 보증금은 **품목 종류의 능력 값이 아니라 규칙 행**(`deposit_rules`)이다: 금액 · 받는 때 · 돌려드리는 법 · 안 돌아올 때가 한 덩이이고, 금액은 시즌마다 바뀔 수 있다. 가게 규칙 화면의 `리프트권 보증금` 카드(`보증금`: `받음` · `안 받음` · `1매 5,000원 ›`, `받는 때`: `접수할 때 / 못 받으면 권을 줄 때` · `권을 줄 때`, `안 돌아오면`: `보증금 안 돌려드림` · `권 값 받음 / 보증금만큼 빼고`; 둘째 판 시안 V8, ui 6-9)가 이 행을 쓴다. 카드는 규칙 행마다 한 장이다.

- **규칙**: 대상(종류 전체 또는 상품 하나 — 상품 규칙이 먼저), 매당 금액, 받는 때(`접수할 때` = at_intake · `권을 줄 때` = at_issue), 결제 칸(확정 창의 `리프트권 보증금` 칸), 돌려드리는 법의 기본(`현금으로 돌려드림` · `미수에서 빼기` · `받은 수단으로`), 안 돌아올 때(`보증금 안 돌려드림` = keep · `권 값 받음 / 보증금만큼 빼고` = charge_loss + 분실 값), 며칠 뒤(`unreturned_after_days`: 비우면 사람이 정함, 0 = 반납 약속일의 마감, 1 = 다음 영업일의 마감), 효력 기간. 규칙이 없으면 보증금이 없다. 이 매장의 값은 3절(1매 5,000원, 접수할 때, 현금, 보증금 안 돌려드림, 1일). 기능 `deposits`가 꺼지면 규칙은 쓰이지 않는다.
- **받기**: 받는 때가 `접수할 때`면 확정 창의 보증금 칸(현금 기본, 매수 × 금액, 할인 · `나중에` 없음)이 한 건을 받는다(`order.create` 안의 `deposit.take`). 그때 받지 못한 팀(전화 예약: 리프트권 값만 계좌이체로 선입금)은 지급 창이 권을 주기 전에 먼저 묻는다. `권을 줄 때`인 매장은 보증금 칸이 확정 창이 아니라 지급 창에 나온다. 차량이 현장에서 권을 더하면(`field.add_ticket`) 같은 명령이 차량 지갑으로 보증금까지 받는다. 돈은 `payments(deposit_in, 목적 deposit)` 한 건, 매수는 `deposit_entries(take)`. 보관 머리(`deposits`)는 **팀 · 규칙마다 하나**이고 규칙 값을 복사한다(시즌 중에 금액을 바꿔도 이 팀에 말한 금액대로 돌려드림).
- **돌려드리기(매수만큼)**: 권이 돌아오는 반납(`direct_return`, 차량 `collect`)과 함께 그 매수의 보증금만 돌려드린다. 3매 중 2매가 오면 10,000원(부분 반납, data-model 4-18). 돈은 한 건(`deposit_out`, 카운터 돈통 또는 차량 지갑)이고 장부 줄은 권마다 그 이동 줄을 가리킨다. 반납 창의 `돌려드리는 방법`에서 `미수에서 빼기`를 고르면 `deposit_apply`(청구를 갚는 돈, 돈통이 움직이지 않음)이고 미수보다 많은 보증금은 현금으로 돌려드린다. 카드로 받은 보증금은 `받은 수단으로`(카드 부분 취소 요청)도 된다. 기사가 수거 현장에서 돌려드리는 것은 `field.deposit_return`(오프라인 허용, 돈이라 거절하지 않음; 그 자리에서 돌려드릴지는 README 열린 질문 16).
- **안 돌아온 권**: 반납 약속이 지나면 마감 인계에 `안 돌아온 리프트권 · N매 · 늦음`과 `보증금 보관 중`으로 넘어간다. 규칙의 며칠이 차면 그 마감이(미리 보기 `마감하면 최하은 · 0026 권 1매 보증금 5,000원을 돌려드리지 않습니다`, 또는 사람이 `보증금에서 빼기`로 — 버튼 말은 README 열린 질문 25) 폐기 이동과 `deposit_forfeit`(보증금 안 돌려드림: 매장이 가짐, 현금은 받을 때 이미 돈통에 들어옴)을 적는다. `권 값 받음 / 보증금만큼 빼고`(`charge_loss`) 규칙이면 분실 조정(`loss_amount` × 매수, `adjustment_assets`)을 청구하고 보증금으로 먼저 낸다(`deposit_apply`). 뒤늦게 돌아오면 찾음 이동 + `deposit_restore` + 돌려드림이고, 돌려드릴지는 사람이 정한다(`deposit_kept_returned`).
...
- 칸마다(채택한 둘째 판 시안 V4): 1줄 = 글만(칸 이름 · 품목 · 할인 상태 · 결제 예정 · 받을 금액 — 기본 = 칸 합계, 전화 예약의 선입금 설정이면 그 값), 2줄 = 빠른 수단 버튼(`payment_methods`의 `quick = 1`, 등급마다 최대 4) + **`나중에`**(N4) + `다른 수단`(간편결제 · 상품권 · 외상 같은 나머지) + 끝에 `할인 고르기 ›`(그 묶음의 할인). 수단이 하나뿐인 칸은 버튼 없는 한 줄.
- `나중에`를 고르면 `결제할 팀` 줄이 보인다(자리는 늘 있음): `이민호 · 0042`(이 팀) · 다른 팀 · `다른 팀 고르기 · 끝 4자리` → `payment_promises(by_other_order)`. 그 칸 제목과 그 팀 카드에는 '이정호 팀 결제 예정'(N19). `나중에`인 칸 모두에 걸린다.
- 주 버튼 문구는 실제로 받는 금액만, 수단 이름과 함께 보인다(`돈 받고 접수 확정 · 현금 160,000원`, 섞이면 `돈 받고 접수 확정 · 카드 225,000원 · 현금 160,000원`, 받는 돈이 없으면 `접수 확정`). 보증금은 수납이 아니라서 `수납하고`를 쓰지 않는다(옛 글 `수납하고 접수 확정 · 35,000원`을 고침). 그 위의 `지금 받는 돈` 줄이 수납과 보증금을 나눠 적는다(`현금 160,000원 = 리프트권 140,000원 + 보증금 20,000원`).
- **보증금 칸**(2026-09-24): 보증금 규칙의 결제 칸(`lift_deposit`)은 칸 합계 대신 `매수 × 매당 금액`을 받고(`4매 · 이 매장은 1매 5,000원 · 권을 돌려받으면 돌려드림` · `현금 20,000원`) 할인과 `나중에`가 없다. 이 매장은 수단이 현금 하나라 버튼 없는 한 줄(52px)이고, 장비 · 리프트권 두 줄 칸과 함께 창이 504px라 1024×529에서도 한 쪽이다(ui 4-5). 주 버튼 금액에 보증금도 들어간다(`돈 받고 접수 확정 · 현금 160,000원` = 권 140,000 + 보증금 20,000). 돈은 칸마다 한 건이다.
...
| 리프트권은 `order.issue`로 지급 못 하고 권 배정이 먼저 | 계열 `ticket` + `ticketed`(도장 사슬 `발권 → 지급`). 끊긴 카운터는 배정과 지급을 한 사실로 적는 권 주기(`ticket.hand_out`, 11절) |
