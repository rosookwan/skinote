...
- **보증금은 매장이 고르는 규칙 행**(`deposit_rules`)이고 매수로 센다(11-1). 이 매장은 리프트권을 꼭 돌려받고 1매 5,000원을 맡는다. 보증금은 청구 · 매출이 아니다.
...
| `return_policy_key` | required · optional · none | **반납이 필요한가.** required: 돌아와야 줄이 끝남. optional: 지급으로 끝, 회수하면 기록하고 반납 도장. none: 반납 칸이 `—`. 리프트권은 **매장이 고른다**(운영 규칙 `리프트권 반납`: `반납 필수` = required · `반납 선택 · 반납 시 기록` = optional; 둘째 판 시안 V8의 자리, 말은 README D6). 이 매장은 required(2026-09-24) | 줄 완료, 반납 도장, 수거 목록 |
...
**보증금은 능력 값이 아니라 규칙 행이다**(`deposit_rules`, 11-1): 매당 금액 · 받는 때 · 결제 칸 · 돌려드리는 법 · 안 돌아올 때 · 며칠 뒤가 한 덩이이고, 금액이 시즌마다 바뀔 수 있어 효력 날짜가 있으며, 종류 전체나 상품 하나를 대상으로 한다. 반납 정책(돌아와야 하나)과 보증금(돈을 맡나)은 따로 고른다: 꼭 돌려받지만 보증금이 없는 매장, 주면 끝이지만 돌려주면 보증금을 돌려드리는 매장도 된다.
...
| lift_deposit | 리프트권 보증금(기능 deposits; 보증금 규칙의 칸, 할인 없음, `후불` 없음 — 받지 못한 팀은 지급 창이 먼저 물음. 이 매장은 수단이 현금 하나라 확정 창에서 버튼 없는 한 줄, 15절) | 현금만 · 접수 시 | — |
...
| lift_ticket | 리프트권 | ticket | unit | **required**(사장님 답 2026-09-24: 꼭 돌려받음. 다른 매장은 optional을 고를 수 있음) | per_unit | lift | agency(세무 확인 전까지 NULL) | ticketed, ends_same_day, delivery, 기능 lift_tickets, 타일 grouped, 정산 resort_vendor, 보증금 규칙 `lift_ticket_card` | 발권, 지급, 반납 |
...
**보증금 규칙**(`deposit_rules`, 11-1): `lift_ticket_card` · '리프트권 보증금' · 대상 종류 `lift_ticket` · **1매 5,000원(이 매장 값)** · 받는 때 `at_intake`(화면 `접수 시 / 미입금: 지급 시`: 전화 예약처럼 그때 못 받으면 지급 창이 먼저 물음) · 결제 칸 `lift_deposit` · 돌려드리는 법 `cash`(`현금 반환`; 미수가 있으면 반납 창의 `반환 방법`에서 `미수 차감`을 고를 수 있음) · 안 돌아오면 `keep`(화면 `보증금 몰수`) · `unreturned_after_days` 1(반납 약속일 다음 영업일의 마감이 정리; 사장님 확인 전 시작 값, README 열린 질문 17). 기능 `deposits` 켬.

**매장 값**: 하루 기준 시각(운영 규칙 화면 `영업일 기준 시각` · `06:00`) `shops.business_day_cutoff = '06:00'`(스키 매장 기본값; data-model 3-3), 선입금 방식 `full_lift_ticket`, 당일 취소 환불 `refund`, `기사 화면 미수 표시` 켬. 모두 관리 → 매장 설정 → 운영 규칙에서 바꾸고, 바꾸면 그다음 기록부터다.
...
12월 26일(토) 전화 예약, 스키 2 · 보드 1 · 헬멧 3 · 야간권 성인 3매. 리프트권 105,000원은 12월 24일 계좌이체로 선입금, 장비 120,000원은 미수, 반납 22:00 설천 주차장 · 1호 차량(redesign 시안 C2와 같은 자료). 2026-09-24 사장님 답으로 권도 반납 약속과 보증금(3매 15,000원)을 가진다.
...
- 종류 `lift_ticket`: 계열 `ticket`, 번호 관리, 반납 정책은 매장이 고름(이 매장 `required`), `per_unit`, 결제 칸 · 할인 묶음 lift, `ticketed`, `ends_same_day`, 기능 `lift_tickets`, 고르기에서 한 타일 뒤에 권종 창(`grouped`), 보증금은 규칙 행(11-1).
...
- 예비권: 어느 claim에도 묶이지 않고 매장이나 차량 위치에 있는 권. 차량 예비권으로 현장에서 권 추가(`field.add_ticket`)는 그 차량 위치에 있는 권만. **끊긴 카운터의 권 지급**(`ticket.hand_out`, 2026-09-24 셋째 검토): 배정(`ticket.allocate`)은 결정이라 연결이 필요하므로, 끊긴 카운터는 권 줄의 지급 칸에서 **매장 권 재고**의 번호를 골라 배정 claim(권 공용 창)과 지급 이동을 한 사실로 적는다(`field.add_ticket`과 같은 모양, 제한 `shop_ticket_stock`: 사본의 권 공용 창으로 사용 창이 겹치지 않는 번호만). 그 번호가 그사이 다른 팀에 배정됐으면 이동은 적고 부딪힌 배정은 `displaced_by_fact`(sync 8-3). 그래서 끊긴 동안 판 권도 번호가 적혀 보증금 장부 · 반납 창 · 발권처 환불이 맞는다.
...
- **반납 정책은 매장이 고른다**(사장님 답 2026-09-24: 이 매장은 꼭 돌려받는다 — 리프트권 보증금이 있어서. 다른 스키장 · 매장은 그렇지 않을 수 있다). 고르는 곳은 운영 규칙 `리프트권 반납` 하나(종류의 `return_policy_key`)이고, 줄은 만들 때 정책을 복사하므로 이미 받은 접수는 바뀌지 않는다.
  - `required`(이 매장): 권 줄에 반납 약속과 반납 도장이 있고, 권이 돌아오거나 미반납(보증금에서 뺀 권의 폐기 이동)이 될 때까지 줄 · 접수가 끝나지 않는다. 손님이 권만 먼저 돌려주면 권 줄의 이동 줄만 생긴다(data-model 4-18). 옛 시스템도 전달한 권을 회수할 때까지 손님 보유로 셌으므로 이 매장은 옛 동작과 같다(migration 3-6).
...
사장님 답(2026-09-24): 리프트권은 꼭 돌려받는다. 리프트권 보증금이 있어서다. 다른 스키장은 그렇지 않을 수 있으니 매장이 고르게 한다. 보증금은 **품목 종류의 능력 값이 아니라 규칙 행**(`deposit_rules`)이다: 금액 · 받는 때 · 돌려드리는 법 · 안 돌아올 때가 한 덩이이고, 금액은 시즌마다 바뀔 수 있다. 운영 규칙 화면의 `리프트권 보증금` 카드(`보증금`: `사용` · `미사용` · `1매 5,000원 ›`, `입금 시점`: `접수 시 / 미입금: 지급 시` · `지급 시`, `미반납 시`: `보증금 몰수` · `분실금 청구 / 보증금 공제`; 둘째 판 시안 V8, ui 6-9)가 이 행을 쓴다. 카드는 규칙 행마다 한 장이다.
...
- **보증금 칸**(2026-09-24): 보증금 규칙의 결제 칸(`lift_deposit`)은 칸 합계 대신 `매수 × 매당 금액`을 받고(`4매 · 매장 기준 1매 5,000원 · 반납 시 반환` · `현금 20,000원`) 할인과 `후불`이 없다. 이 매장은 수단이 현금 하나라 버튼 없는 한 줄(52px)이고, 장비 · 리프트권 두 줄 칸과 함께 창이 504px라 1024×529에서도 한 쪽이다(ui 4-5). 주 버튼 금액에 보증금도 들어간다(`접수 확정 · 현금 160,000원` = 권 140,000 + 보증금 20,000). 돈은 칸마다 한 건이다.
