...
작성 2026-09-23(같은 날 검토 반영). 고침 2026-09-24: 사장님 답 — 리프트권은 꼭 돌려받고(이 매장 `required`) 권 보증금이 있다(11-1, 다른 매장은 고름), 하루 기준 시각 06:00(3절). 같은 날 채택한 둘째 판 시안의 화면 말(가게 규칙 카드 V8, 확정 창 V4, 반납 창 V1)에 맞췄고, 끊긴 카운터의 새 접수도 기기의 게시 판 값으로 매긴다(8절, README ADR-19). 셋째 검토(같은 날): 큐에서 온 새 접수의 가격은 돌려보내지 않음(8절), 끊긴 카운터의 권 주기(11 · 16절), 한도 넘는 할인은 비밀번호로 승인(9절). **같은 날 넷째 고침:** 인용한 화면 말을 짧은 업무 용어로 바꿨다(README D6, [docs/design/wording.md](../design/wording.md): 운영 규칙 카드 · 확정 창 · 반납 창 · 보증금 말). **고침 2026-09-26(첫 매장 답, README D7):** 첫 매장은 장비 · 리프트권에 번호가 없어 모든 종류를 수량(`count`)으로 세고(2 · 3 · 4 · 11절), 사장님이 말한 권 '보증금'은 스키장 권 값 안의 카드 보증금(1,000원)이라 **보증금 규칙 행이 없다**(3 · 11 · 11-1 · 15절). 권은 계속 꼭 돌려받되 안 돌아와도 손님에게 청구하지 않는다. 결제 수단 · 당일 취소 · 노쇼 환불도 3 · 14절에 적었다. 보증금 규칙의 구조와 예시(1매 5,000원)는 손님 보증금을 받는 다른 매장을 위해 그대로 둔다. 품목 종류, 능력 값, 상품과 규격, 속성, 세트, 요금표, 할인, 가격 출처, 리프트권과 강습, 보증금, 세금을 정한다. 표 정의는 [`schema.sql`](../../packages/schema/schema.sql) 2.4 · 2.5절, 전체 원칙은 [data-model.md](data-model.md).
...
- **보증금은 매장이 고르는 규칙 행**(`deposit_rules`)이고 매수로 센다(11-1). 보증금은 청구 · 매출이 아니다. 예시 값은 리프트권 1매 5,000원이다. **첫 매장은 규칙 행이 없다**(2026-09-26, README D7): 사장님이 말한 1,000원은 스키장 권 값 안의 카드 보증금이라 손님에게 받거나 돌려주는 돈이 아니다. 첫 매장은 리프트권을 꼭 돌려받되(`required`) 안 돌아와도 손님에게 청구하지 않는다.
- **재고 방식은 매장 값이다**(`tracking_key`): 번호 스티커가 있으면 `unit`, 없으면 `count`. 첫 매장은 장비 · 리프트권 모두 번호가 없어 모든 종류가 `count`다(2026-09-26, README D7).
...
| `tracking_key` | unit · count · none | 재고를 하나씩(`assets`) 세는지, 수량으로(`stock_balances`) 세는지, 세지 않는지. **첫 매장은 모든 종류 count**(번호 스티커 · 권 번호가 없음, 2026-09-26 README D7) | 지급 · 반납 창(장비 고르기 또는 수량), 재고 화면, 차량 적재 요약 |
| `return_policy_key` | required · optional · none | **반납이 필요한가.** required: 돌아와야 줄이 끝남. optional: 지급으로 끝, 회수하면 기록하고 반납 도장. none: 반납 칸이 `—`. 리프트권은 **매장이 고른다**(운영 규칙 `리프트권 반납`: `반납 필수` = required · `반납 선택 · 반납 시 기록` = optional; 둘째 판 시안 V8의 자리, 말은 README D6). 이 매장은 required(2026-09-24; 2026-09-26 D7에도 그대로 — 카드를 모아 스키장에 돌려줌, 보증금은 없음) | 줄 완료, 반납 도장, 수거 목록 |
...
| `requires_headcount` | 0/1 | 인원을 받아야 함(강습, 인원 예약) | 접수 창의 인원 입력 |
...
**보증금은 능력 값이 아니라 규칙 행이다**(`deposit_rules`, 11-1): 매당 금액 · 받는 때 · 결제 칸 · 돌려드리는 법 · 안 돌아올 때 · 며칠 뒤가 한 덩이이고, 금액이 시즌마다 바뀔 수 있어 효력 날짜가 있으며, 종류 전체나 상품 하나를 대상으로 한다. 반납 정책(돌아와야 하나)과 보증금(돈을 맡나)은 따로 고른다: 꼭 돌려받지만 보증금이 없는 매장(**첫 매장**, README D7), 주면 끝이지만 돌려주면 보증금을 돌려드리는 매장도 된다.
...
|---|---|---|---|
| gear | 장비 | 카드 · 접수 시 | per_unit_day(개당 1일), percent, amount, manual_amount |
| lift | 리프트권 | 현금 · 접수 시 | percent |
| ~~lift_deposit~~ | 리프트권 보증금(기능 deposits; 보증금 규칙의 칸, 할인 없음, `후불` 없음 — 받지 못한 팀은 지급 창이 먼저 물음. 수단이 현금 하나면 확정 창에서 버튼 없는 한 줄, 15절). **첫 매장은 두지 않는다**(보증금 규칙이 없음, 2026-09-26 README D7). 손님 보증금을 받는 매장의 칸이다 | 현금만 · 접수 시 | — |
| lesson | 강습(기능 lessons, 꺼짐) | 현금 · 접수 시 | percent, amount |

**품목 종류**

| key | 이름 | 계열 | 재고 | 반납 | 가격 | 칸 · 할인 | 세금 | 능력 값 | 도장 |
|---|---|---|---|---|---|---|---|---|---|
| equipment | 장비 | rental | **count**(번호 없음, D7) | required | per_day | gear | taxable | sized, exchangeable, extendable, preparable, delivery | (적재 →) 지급, 반납 |
| clothing | 의류 | rental | **count** | required | per_day | gear | taxable | sized, exchangeable(N17), extendable, preparable, delivery | (적재 →) 지급, 반납 |
| helmet | 헬멧 | rental | **count** | required | per_day | gear | taxable | sized, exchangeable(N17), extendable, preparable, delivery | (적재 →) 지급, 반납 |
| lift_ticket | 리프트권 | ticket | **count**(권 번호 없음, D7) | **required**(사장님 답 2026-09-24: 꼭 돌려받음. 2026-09-26 D7: 카드를 스키장에 돌려주려고 돌려받지만 보증금은 없고, 안 돌아와도 손님에게 청구하지 않음. 다른 매장은 optional을 고를 수 있음) | per_unit | lift | agency(세무 확인 전까지 NULL) | ticketed, ends_same_day, delivery, 기능 lift_tickets, 타일 grouped, 정산 resort_vendor. 보증금 규칙 없음(D7) | 발권, 지급, 반납 |
...
| headcount | 인원 예약 | placeholder | none | none | flat(0원) | gear | — | requires_headcount, 선택 목록 제외(`hidden`) | — |

세금 분류 값(특히 리프트권 대행과 강습)은 세무 담당 확인 뒤 종류 값만 채운다(README 열린 질문). 비워 두면 줄에 세금이 기록되지 않을 뿐 다른 것은 그대로 돈다.

재고 방식은 2026-09-26 README D7로 모두 `count`다(처음 표는 `unit`이었다: 사장님 답 전의 가정). 부츠 · 고글을 따로 빌려주는지는 아직 확인하지 않았다(둘째 판 시안 명세 6절 질문 3) — 빌려주면 같은 `count` 종류 행을 더한다. 수량 품목은 규격 행이 적어도 하나다(`stock_balances`의 키가 규격이다): 의류 · 헬멧은 사이즈마다, 규격을 가르지 않는 스키 · 보드와 권종은 규격 하나로 센다(길이별로 세고 싶어지면 규격 행을 더한다).

**보증금 규칙 — 첫 매장은 없음**(2026-09-26, README D7): 사장님이 말한 1,000원은 스키장이 권 값 안에 넣은 카드 보증금이고, 가게가 카드를 돌려주면 스키장이 가게에 돌려준다. 손님과 주고받는 돈이 아니므로 규칙 행 · 보증금 결제 칸이 없고, 시스템은 스키장과의 1,000원을 적지 않는다. 운영 규칙의 `리프트권 보증금` 카드는 `미사용`이다. 아래는 사장님 답 전(2026-09-24)에 이 매장 값으로 적었던 규칙이고, 손님 보증금을 받는 매장의 예로 남긴다: `lift_ticket_card` · '리프트권 보증금' · 대상 종류 `lift_ticket` · **1매 5,000원(예시 값)** · 받는 때 `at_intake`(화면 `접수 시 / 미입금: 지급 시`: 전화 예약처럼 그때 못 받으면 지급 창이 먼저 물음) · 결제 칸 `lift_deposit` · 돌려드리는 법 `cash`(`현금 반환`; 미수가 있으면 반납 창의 `반환 방법`에서 `미수 차감`을 고를 수 있음) · 안 돌아오면 `keep`(화면 `보증금 몰수`) · `unreturned_after_days` 1(반납 약속일 다음 영업일의 마감이 정리; README 열린 질문 17). 규칙이 없으면 기능 `deposits`의 켬 · 끔과 상관없이 보증금이 없다.

**매장 값**: 하루 기준 시각(운영 규칙 화면 `영업일 기준 시각` · `06:00`) `shops.business_day_cutoff = '06:00'`(스키 매장 기본값; data-model 3-3), 선입금 방식 `full_lift_ticket`, 당일 취소 환불 `refund`(2026-09-26 D7 확인: 거의 돌려주지만 너무 늦게 연락 오면 직원이 취소 창에서 `환불 없음`을 고름 — 14절), `기사 화면 미수 표시` 켬. 모두 관리 → 매장 설정 → 운영 규칙에서 바꾸고, 바꾸면 그다음 기록부터다. 노쇼(연락 없이 안 옴)도 같은 취소 창의 환불 결정이다: 기본 `환불`, 그날 잠수 타고 다음 날 온 손님은 직원이 `환불 없음`(D7). 노쇼만 가르는 설정은 두지 않는다.

**결제 수단**(`payment_methods`, 2026-09-26 D7): 빠른 수단 `카드` · `현금` · `계좌이체`(`quick = 1`, 99%)와 `기타`. 지역화폐 · 상품권 행은 두지 않고, `기타` 판의 행은 시드 때 여쭌다(README 열린 질문 37). 기사가 현장에서 받는 수단(`driver_allowed`)은 `현금` · `계좌이체`뿐이고 안 되면 `후불`(매장에서 받음). 카드 단말기는 1대이고 연동하지 않는다(sync 8-10).
...
**도장 단계**: `issue` 지급 · `return` 반납 · `pay` 수납 · `load` 적재 · `ticket_secure` 발권 · `collect` 수거(시각 표시) · `receive` 매장 입고 · `lesson_done` 강습(기능 lessons). 단계마다 동작(`sys_actions`: 명령 + 확인 창 + 되돌리기)이 하나다. 리프트권 종류는 이 매장에서 `return` 단계도 가진다(`item_kind_stamp_steps` 행 하나). 반납 정책이 optional인 매장은 회수한 권에만 반납 도장이 찍히고 회수 전에는 반납 칸이 `—`다.
...
- **첫 매장에는 번호로 관리하는 품목이 없다**(2026-09-26, README D7): 스티커 번호도 권 번호도 없어 모든 종류가 수량 품목이다. 번호 없는 `unit`(장비 행은 있고 스티커가 없음)으로 두지 않은 까닭: 반납 · 지급 창과 기록에 실물에 없는 번호가 생기고, 직원이 어느 것이 돌아왔는지 가를 수 없다. 나중에 스티커를 붙이면 그 종류만 `unit`으로 바꾸고 그 종류의 수량을 장비 행으로 옮긴다(줄은 만들 때 재고 방식을 복사하므로 지난 줄은 그대로; 수량 → 장비 행 명령은 그때 만든다).
...
- **오프라인 기록의 가격**(차량의 권 추가, 그리고 2026-09-24 ADR-19부터 끊긴 카운터의 새 현장 접수 · 품목 추가 · 권 지급): 기기가 가진 판으로 계산해 판 id · 금액 · `quote_hash`를 함께 보낸다. 서버는 그 판이 `occurred_at`에 효력이 있던 게시 판이면 그대로 받고(`price_source_key 'offline_quote'`), 아니면 기기 값으로 적고 `확인 필요`에 올린다. 권 추가는 사실(fact)이고, 새 접수 · 품목 추가는 등급이 intent여도 **큐에서 온 명령은 적어 두는 사실**로 다룬다(sync 8-12). 그래서 어느 쪽도 `QUOTE_CHANGED`로 돌려보낼 수 없고, 조용히 다시 계산하지도 않는다(현장에서 받은 돈과 청구가 어긋나지 않게). 온라인으로 바로 보낸 새 접수 · 품목 추가만 `QUOTE_CHANGED`를 받는다.
...
지금 규칙(docs/44): 한 묶음에 할인 하나, 겹치지 않음. 장비 묶음은 개당 1일 금액(금액 × 수량 × 일수, 총액까지) · 장비 합계의 % · 장비 합계에서 금액, 리프트권 묶음은 %. 결과는 10원 단위로 내리고, 묶음 안 줄들에 총액 비율로 나누며, 줄마다 몫도 10원 단위로 내리고, 남은 끝전은 큰 줄부터 준다(옛 `applyDiscounts`와 같은 계산, 순수 함수로 옮김).
...
- 역할별 한도(`role_permissions.limits_json`의 `max_discount_amount` · `max_discount_percent_bp`)를 넘으면 관리자가 **비밀번호나 휴대폰 확인**으로 승인한다(PIN이 아님: 기기에 둔 PIN 확인 값으로 돈 통제를 풀 수 없게, sync 7절). 할인 직접 입력은 연결이 필요한 일이다.
- 보고서는 내역을 상품 · 규칙 · 기간별로 묶는다(할인 종류별 합계, 거래처별 절감액).

## 10. 줄에 남는 가격 출처 — 박준호 팀 예시

12월 26일(토) 전화 예약, 스키 2 · 보드 1 · 헬멧 3 · 야간권 성인 3매. 리프트권 105,000원은 12월 24일 계좌이체로 선입금, 장비 120,000원은 미수, 반납 22:00 설천 주차장 · 1호 차량(redesign 시안 C2와 같은 자료). 2026-09-24 사장님 답으로 권도 반납 약속과 보증금(3매 15,000원)을 가진다. **첫 매장(2026-09-26, README D7)이면** 권 줄의 반납 약속은 같고, 보증금 행(아래 `deposits` · `payments` · `deposit_entries` 줄, 결제 약속의 보증금 칸, 투영의 `deposit_held_amount`)이 없다. 청구 · 수납 · 미수는 같다. 아래 표는 보증금 규칙이 있는 매장의 예로 둔다.

| 표 | 행 |
|---|---|
| `order_lines` | L1 스키: rental · required · per_day, 수량 2, 1일, 단가 40,000, 총액 80,000, 할인 0, 순액 80,000, 판 `std-v1`, 규칙 `pr-ski`, `per_day@1` · L2 보드: 25,000 × 1 = 25,000 · L3 헬멧: 5,000 × 3 = 15,000 · L4 야간권 성인: ticket · required · per_unit, 35,000 × 3 = 105,000, 사용 창 18:30~22:00, 발권 예정일 12/26 |
| `line_price_components` | L1 `base` '스키 1일 40,000원 × 2대 × 1일' 80,000 · L2 `base` 25,000 · L3 `base` 15,000 · L4 `base` '야간권 성인 35,000원 × 3매' 105,000 |
| `line_promises` | L1~L4 수령 `shop_counter` 12/26 16:00 · 반납 `vehicle_collection` 12/26 22:00 설천 · 설천 주차장 · van-1 (반납 타임 '야간타임 후' 스냅샷). L4 권도 반납 약속이 있다(이 매장은 required; optional 매장이면 권 줄의 반납 약속이 없다) |
| `payment_promises` | 리프트권 칸 `prepayment` · `at_intake` · 105,000(`prepayment_mode = full_lift_ticket`에서 자동) · 장비 칸 `charge` · `at_issue` · 리프트권 보증금 칸 `deposit` · `at_intake`(전화 예약이라 그때 받지 못함 → 지급 창이 먼저 물음) |
...
| `deposits` · `payments` · `deposit_entries` | 16:05 지급 때: 보관 하나(팀 · 규칙 `lift_ticket_card`, 1매 5,000원 복사) · `deposit_in` 현금 15,000(목적 `deposit`, 카운터 돈통) · 장부 `take` L4 3매 15,000. 청구 · 미수에는 들지 않는다 |
| `orders` 투영 | 청구 225,000 · 수납 105,000 · 미수 120,000 · 보증금 15,000(`deposit_held_amount`) · 수납 상태 partial. 카드 문구 '예약금 105,000원 수납 · 미수 120,000원', 접수증 돈 줄 끝에 `보증금 15,000원` |
...
- 종류 `lift_ticket`: 계열 `ticket`, 재고 방식은 매장이 고름(번호 `unit` 또는 수량 `count`; **첫 매장은 `count`**, 2026-09-26 README D7), 반납 정책은 매장이 고름(이 매장 `required`), `per_unit`, 결제 칸 · 할인 묶음 lift, `ticketed`, `ends_same_day`, 기능 `lift_tickets`, 고르기에서 한 타일 뒤에 권종 창(`grouped`), 보증금은 규칙 행(11-1, 첫 매장은 없음).
...
- **수량으로 세는 권(첫 매장, D7):** 권 번호를 적지 않으므로 권 한 장마다의 행(`assets` · `ticket_units`)이 없다. 권종(상품)마다 매장 · 차량 위치의 수량(`stock_balances`)이고, 사용 창 · 유효 일수는 권종(`ticket_products`)에서 온다. 미리 끊어 둔 권은 재고 입고(`stock.intake`)로 매장에 들어오고, 확보(`발권` 도장)는 그 권종 수량을 잡아 두는 것(`stock_holds`, 접수 줄), 지급은 매장 → 손님 수량 이동이다. **돌아온 권 카드는 다시 줄 수 없으므로** 지급할 수 없는 상태(매장의 `asset_conditions` 행, `issuable = 0`)로 매장에 들어와 예비권 수에 섞이지 않는다. 가게가 모은 카드를 스키장에 돌려주고 1,000원을 받는 일은 시스템이 적지 않는다(적고 싶어지면 폐기 · 거래처 이동과 거래처 정산으로 된다). 이 상태의 화면 이름이 필요해지면 wording.md에 먼저 넣는다.
- 수요: 접수 줄(사용일, 사용 창, **발권 예정일** ≠ 사용일). 확보: 시간대가 있는 `line_fulfillment` claim(차선 `ticket_window`). 확보 수 · 아직 필요한 수 · 취소 수는 줄 투영과 claim으로 계산.
- 예비권: 어느 claim에도 묶이지 않고 매장이나 차량 위치에 있는 권. 차량 예비권으로 현장에서 권 추가(`field.add_ticket`)는 그 차량 위치에 있는 권만. 첫 매장은 손님이 몰리는 날 차량에 예비권을 미리 싣는다(D7 답 14): 차량 위치의 권종별 수량에서 꺼내고 보증금은 없다. 싣고 내리는 명령(`차량 예비권 적재`)은 후속이라(roadmap 3절) 지금은 매장을 만들 때의 처음 수량뿐이다. **끊긴 카운터의 권 지급**(`ticket.hand_out`, 2026-09-24 셋째 검토): 배정(`ticket.allocate`)은 결정이라 연결이 필요하므로, 끊긴 카운터는 권 줄의 지급 칸에서 **매장 권 재고**의 번호를 골라 배정 claim(권 공용 창)과 지급 이동을 한 사실로 적는다(`field.add_ticket`과 같은 모양, 제한 `shop_ticket_stock`: 사본의 권 공용 창으로 사용 창이 겹치지 않는 번호만). 그 번호가 그사이 다른 팀에 배정됐으면 이동은 적고 부딪힌 배정은 `displaced_by_fact`(sync 8-3). 그래서 끊긴 동안 판 권도 번호가 적혀 보증금 장부 · 반납 창 · 발권처 환불이 맞는다. **수량으로 세는 매장(첫 매장)은** 번호를 고르지 않고 매장 권 재고에서 그 권종 수량을 꺼내 지급 이동으로 적는다(제한 `shop_ticket_stock`은 사본의 그 권종 지급 가능 수량을 본다). 그사이 서버에서 그 수량이 다른 팀 확보로 잡혔으면 이동은 적고 부딪힌 확보를 푼다(사실이 계획보다 이김, sync 8-3).
...
- 회수한 권(번호 권 매장): 다음 손님에게 다시 주면 새 claim(`source_key = 'recovered_ticket'`). 첫 매장은 쓰다 만 권을 되팔거나 발권처에 환불받는 일이 거의 없어(D7 답 5) 이 화면들을 만들지 않는다(표는 그대로). 발권처에 돌려주면 `vendor_refunds`(차선 `disposition`, 한 권은 살아 있는 환불 하나에만) → 방문마다 `vendor_refund_attempts` → 받은 돈은 `counterparty_settlements(receipt)`의 돈통 **한 곳만** `cash_movements` → 마감(시도 행은 현금 출처가 아님). 옛 시스템에서 마감 밖에 있던 돈이 이제 한 번만 들어간다.
- 선입금: 이 매장은 `prepayment_mode = full_lift_ticket`이라 전화 예약의 리프트권 칸이 전액 선입금으로 미리 선택된다. 다른 매장은 금액 또는 없음.
- **반납 정책은 매장이 고른다**(사장님 답 2026-09-24: 이 매장은 꼭 돌려받는다 — 리프트권 보증금이 있어서. 2026-09-26 D7: 그 보증금은 스키장 카드 보증금이라 손님 보증금은 없고, 카드를 스키장에 돌려주려고 꼭 돌려받는다. 다른 스키장 · 매장은 그렇지 않을 수 있다). 고르는 곳은 운영 규칙 `리프트권 반납` 하나(종류의 `return_policy_key`)이고, 줄은 만들 때 정책을 복사하므로 이미 받은 접수는 바뀌지 않는다.
  - `required`(이 매장): 권 줄에 반납 약속과 반납 도장이 있고, 권이 돌아오거나 미반납(보증금에서 뺀 권의 폐기 이동)이 될 때까지 줄 · 접수가 끝나지 않는다. **보증금이 없는 첫 매장(D7)은** 안 돌아온 권을 손님에게 청구하지 않는다(가게가 손실을 짐): 분실금 청구 · 보증금 몰수가 없고, 반납 일정이 지나면 이월 항목 `리프트권 미반납 · N매 · 지연`으로 남는다. 그 줄을 청구 없이 닫는 법(사람이 정리 · 마감이 N일 뒤 정리 · 그대로 둠)은 README 열린 질문 35다. 손님이 권만 먼저 돌려주면 권 줄의 이동 줄만 생긴다(data-model 4-18). 옛 시스템도 전달한 권을 회수할 때까지 손님 보유로 셌으므로 이 매장은 옛 동작과 같다(migration 3-6).
  - `optional`: 권은 지급하면 줄이 끝나고, 돌려받으면 `collect` · `direct_return` 이동으로 기록되고 반납 도장이 찍힌다. 옛 동작과 다르므로 그 매장은 이관 대조의 **의도한 차이 목록**에 올린다.

### 11-1. 리프트권 보증금(보증금 규칙)

**첫 매장은 이 절을 쓰지 않는다**(2026-09-26, README D7). 사장님이 말한 '보증금 1,000원'은 스키장이 권 값 안에 넣은 **권 카드 보증금**이다: 손님은 권 값만 내고, 가게가 카드를 스키장에 돌려주면 스키장이 가게에 1,000원을 돌려준다. 손님이 권을 안 주면 가게가 그 손실을 진다. 손님에게 받거나 돌려주는 돈이 아니므로 첫 매장은 보증금 규칙 행이 없고(확정 창 보증금 칸 · 반납 창 보증금 줄 · 마감 보증금 보관이 없음), 반납 정책 `required`만 남는다. 아래는 손님 보증금을 받는 매장을 위한 규칙이다(예시 값 1매 5,000원).

사장님 답(2026-09-24, 그때의 이해): 리프트권은 꼭 돌려받는다. 리프트권 보증금이 있어서다. 다른 스키장은 그렇지 않을 수 있으니 매장이 고르게 한다. 보증금은 **품목 종류의 능력 값이 아니라 규칙 행**(`deposit_rules`)이다: 금액 · 받는 때 · 돌려드리는 법 · 안 돌아올 때가 한 덩이이고, 금액은 시즌마다 바뀔 수 있다. 운영 규칙 화면의 `리프트권 보증금` 카드(`보증금`: `사용` · `미사용` · `1매 5,000원 ›`, `입금 시점`: `접수 시 / 미입금: 지급 시` · `지급 시`, `미반납 시`: `보증금 몰수` · `분실금 청구 / 보증금 공제`; 둘째 판 시안 V8, ui 6-9)가 이 행을 쓴다. 카드는 규칙 행마다 한 장이다.

- **규칙**: 대상(종류 전체 또는 상품 하나 — 상품 규칙이 먼저), 매당 금액, 받는 때(`접수 시` = at_intake · `지급 시` = at_issue), 결제 칸(확정 창의 `리프트권 보증금` 칸), 돌려드리는 법의 기본(`현금 반환` · `미수 차감` · `동일 수단 반환`), 안 돌아올 때(`보증금 몰수` = keep · `분실금 청구(보증금 공제)` = charge_loss + 분실 값), 며칠 뒤(`unreturned_after_days`: 비우면 사람이 정함, 0 = 반납 약속일의 마감, 1 = 다음 영업일의 마감), 효력 기간. 규칙이 없으면 보증금이 없다. 이 매장의 값은 3절(1매 5,000원, 접수 시, 현금 반환, 보증금 몰수, 1일). 기능 `deposits`가 꺼지면 규칙은 쓰이지 않는다.
- **받기**: 받는 때가 `접수 시`면 확정 창의 보증금 칸(현금 기본, 매수 × 금액, 할인 · `후불` 없음)이 한 건을 받는다(`order.create` 안의 `deposit.take`). 그때 받지 못한 팀(전화 예약: 리프트권 값만 계좌이체로 선입금)은 지급 창이 권을 주기 전에 먼저 묻는다. `지급 시`인 매장은 보증금 칸이 확정 창이 아니라 지급 창에 나온다. 차량이 현장에서 권을 더하면(`field.add_ticket`) 같은 명령이 차량 지갑으로 보증금까지 받는다. 돈은 `payments(deposit_in, 목적 deposit)` 한 건, 매수는 `deposit_entries(take)`. 보관 머리(`deposits`)는 **팀 · 규칙마다 하나**이고 규칙 값을 복사한다(시즌 중에 금액을 바꿔도 이 팀에 말한 금액대로 돌려드림).
- **돌려드리기(매수만큼)**: 권이 돌아오는 반납(`direct_return`, 차량 `collect`)과 함께 그 매수의 보증금만 돌려드린다. 3매 중 2매가 오면 10,000원(부분 반납, data-model 4-18). 돈은 한 건(`deposit_out`, 카운터 돈통 또는 차량 지갑)이고 장부 줄은 권마다 그 이동 줄을 가리킨다. 반납 창의 `반환 방법`에서 `미수 차감`을 고르면 `deposit_apply`(청구를 갚는 돈, 돈통이 움직이지 않음)이고 미수보다 많은 보증금은 현금으로 돌려드린다. 카드로 받은 보증금은 `동일 수단 반환`(카드 부분 취소 요청)도 된다. 기사가 수거 현장에서 돌려드리는 것은 `field.deposit_return`(오프라인 허용, 돈이라 거절하지 않음; 그 자리에서 돌려드릴지는 README 열린 질문 16).
- **안 돌아온 권**: 반납 약속이 지나면 마감 인계에 `리프트권 미반납 · N매 · 지연`과 `보증금 보관 중`으로 넘어간다. 규칙의 며칠이 차면 그 마감이(미리 보기 `최하은 · 0026 권 1매 보증금 5,000원 · 마감 시 몰수`, 또는 사람이 `보증금 몰수`로 — 버튼 · 규칙 · 어휘가 한 말, README 열린 질문 25 · D6) 폐기 이동과 `deposit_forfeit`(`보증금 몰수`: 매장이 가짐, 현금은 받을 때 이미 돈통에 들어옴)을 적는다. `분실금 청구(보증금 공제)`(`charge_loss`) 규칙이면 분실 조정(`loss_amount` × 매수, `adjustment_assets`)을 청구하고 보증금으로 먼저 낸다(`deposit_apply`). 뒤늦게 돌아오면 분실 회수 이동 + `deposit_restore`(`몰수 취소`) + 돌려드림이고, 돌려드릴지는 사람이 정한다(`deposit_kept_returned`).
- **마감**: 보증금은 매출이 아니다. 받은 돈 표에 넣지 않고 돈통 예상에만 들어가며, `보증금 보관 중 · 리프트권 1매 · 5,000원`과 그 팀이 인계로 넘어간다(data-model 4-13). 보증금에서 뺀 돈은 `closing_totals`의 `deposit_forfeit`으로 따로 보이고, 세금에서 어떻게 볼지는 세무 확인이다(README 열린 질문 13).
- **다른 매장 · 다른 품목**: 보증금 없는 매장은 규칙 행이 없다. 장비 보증금(보드 한 대 50,000원), 고글 보증금도 같은 규칙 행 하나이고, 수량 품목은 매수 대신 수량이다. 규칙 없는 보증금(신분증 대신 현금)은 전처럼 보관 머리만 둔다(`deposit_rule_id` 없음).
...
- 취소: 일부 수량도 된다(`partial_cancel_allowed`). 취소 금액은 **그 줄이 실제로 매겨진 내역**(`line_price_components`)에서 비율로 계산한다: `순액 × 취소 수량 / 수량`(10원 내림), 그 줄의 마지막 취소는 남은 금액 전부. 여러 날 합계(2대 × 3일 = 108,000)나 평일 · 주말이 섞인 줄에서도 첫날 단가로 계산하는 잘못(20,000만 빼기)이 없다. 음수 `cancellation` 조정으로 남기고, 취소 줄 · 취소 합계 · 조정의 합이 같은지 검증기가 본다. 발권한 권의 당일 취소는 환불 결정(설정 기본값, 직원이 바꿀 수 있음)을 함께 남긴다. 첫 매장(2026-09-26, README D7): 기본 `환불`이고, 너무 늦게 연락한 당일 취소와 그날 잠수 타고 다음 날 온 노쇼는 직원이 취소 창에서 `환불 없음`을 고른다(환불하지 않으면 받은 돈이 `cancellation_fee`로 청구에 남음, data-model S8).
...
- 칸마다(채택한 둘째 판 시안 V4): 1줄 = 글만(칸 이름 · 품목 · 할인 상태 · 결제 예정 · 받을 금액 — 기본 = 칸 합계, 전화 예약의 선입금 설정이면 그 값), 2줄 = 빠른 수단 버튼(`payment_methods`의 `quick = 1`, 등급마다 최대 4) + **`후불`**(N4) + `기타`(간편결제 · 상품권 · 외상 같은 나머지) + 끝에 `할인 적용 ›`(그 묶음의 할인). 수단이 하나뿐인 칸은 버튼 없는 한 줄.
- `후불`을 고르면 `결제 팀` 줄이 보인다(자리는 늘 있음): `이민호 · 0042`(이 팀) · 다른 팀 · `다른 팀 찾기 · 끝 4자리` → `payment_promises(by_other_order)`. 그 칸 제목과 그 팀 카드에는 '이정호 팀 결제 예정'(N19). `후불`인 칸 모두에 걸린다.
- 주 버튼 문구는 실제로 받는 금액만, 수단 이름과 함께 보인다(`접수 확정 · 현금 160,000원`, 섞이면 `접수 확정 · 카드 225,000원 · 현금 160,000원`, 받을 금액이 없으면 `접수 확정`). 보증금은 수납이 아니라서 `수납하고`를 쓰지 않는다(옛 글 `수납하고 접수 확정 · 35,000원`을 고침). 그 위의 `받을 금액` 줄이 수납과 보증금을 나눠 적는다(`현금 160,000원 = 리프트권 140,000원 + 보증금 20,000원`).
- **보증금 칸**(2026-09-24; **첫 매장은 없음** — 보증금 규칙이 없어 확정 창이 장비 · 리프트권 두 칸이고 받을 금액 줄에 보증금이 없다, 2026-09-26 README D7): 보증금 규칙의 결제 칸(`lift_deposit`)은 칸 합계 대신 `매수 × 매당 금액`을 받고(`4매 · 매장 기준 1매 5,000원 · 반납 시 반환` · `현금 20,000원`) 할인과 `후불`이 없다. 이 매장은 수단이 현금 하나라 버튼 없는 한 줄(52px)이고, 장비 · 리프트권 두 줄 칸과 함께 창이 504px라 1024×529에서도 한 쪽이다(ui 4-5). 주 버튼 금액에 보증금도 들어간다(`접수 확정 · 현금 160,000원` = 권 140,000 + 보증금 20,000). 돈은 칸마다 한 건이다.
...
| 리프트권은 `order.issue`로 지급 못 하고 권 배정이 먼저 | 계열 `ticket` + `ticketed`(도장 사슬 `발권 → 지급`). 끊긴 카운터는 배정과 지급을 한 사실로 적는 권 지급(`ticket.hand_out`, 11절) |
...
| 반납 칸 '—'(시안 C2) | `return_policy_key = none` 또는 optional에서 아직 회수 안 함. 이 매장의 권 줄은 required라 반납 도장이 있다(2026-09-24) |
| 보증금(옛 `finance.js`의 보증금 · 옛 확정 창에 없던 권 보증금) | 보증금 규칙(`deposit_rules`) + 보관 머리 + 보증금 장부(매수), 돈은 `deposit_*` 수납 |
