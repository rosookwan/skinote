...
- **돈과 반납은 어떤 경우에도 막지 않는다**: 지급 · 반납 · 수납 · 환불 · 보증금 · 현장 수납 · 수거 · 매장 입고 · 현금 인계 · 마감 · 인쇄 · 자료 내보내기는 늘 된다(deployment 5-5). **이미 일어난 오프라인 기록은 라이선스 때문에 거절하지 않는다**: 라이선스가 끝난 뒤 시각의 새 접수도 적고 `licence_lapsed_record`로 올린다(sync 8-12). 기기 쪽 확인은 화면 안내용이지 강제가 아니다. 시즌 1(첫 매장)에는 기기 표 · 유예 · 제한의 강제를 켜지 않고 두 번째 매장 전에 켠다.
...
- 부호는 어휘 표가 정한다: `sys_payment_kinds.balance_sign / deposit_sign / cash_sign`, `sys_trade_kinds.direction_sign`, `adjustment_types.sign`. 금액 열은 양수이고 조정(`charge_adjustments.amount`)과 수납 이동의 음수 배분만 부호를 가진다.
...
- **스키 매장의 기본값이자 이 매장의 값은 06:00(새벽 6시)이다**(사장님 답 2026-09-24: 자정을 넘겨도 전날 반납으로 친다). 매장 개설(`shop.provision`)의 시드가 06:00을 넣는다. 열의 기본값 `00:00`은 시드 없이 만든 매장을 위한 기술적인 값일 뿐이다. 06:00으로 정한 까닭: 야간 · 심야 영업과 그 뒤의 수거 · 매장 입고 · 마감이 새벽 2시 전후에 끝나고, 매장이 문을 여는 시각(7시 전후)보다는 이르다. 새벽 6시 전의 대여 · 배달은 거의 없고, 있어도 전날 밤의 일로 세는 편이 가게의 셈과 맞는다. 이보다 늦게(예: 08:00) 잡을 까닭은 찾지 못했다(아침 첫 대여가 전날 장부로 가게 됨).
  ...
  - **마감 화면**은 제목줄의 기준 띠에 어느 날의 장부인지 적는다: `현재 27일 00:40` · `26일 장부 · 기준 06:00 · 00:15 정하늘 · 0041 반납 포함`(채택한 둘째 판 시안 V6, ui 6-8 · 7절, 4-13).
  - **기준 시각을 바꾸면**(관리 → 매장 설정 → 운영 규칙의 `영업일 기준 시각`: `00:00` · `03:00` · `06:00` · `직접 입력`) 그다음 기록부터 새 기준을 쓰고, 저장한 `business_date`는 다시 계산하지 않는다. 지금 시각이 옛 기준과 새 기준 사이이면(예: 02:00에 00:00 → 06:00) 설정 명령이 거절한다(`변경 불가 · 06:00 이후 가능`): 영업일이 거꾸로 가거나 한 시간대가 두 날로 나뉘지 않게. 지점마다 다른 기준 시각이 필요해지면 `closing_scopes`에 빈 값 허용 열을 더한다(E4).
...
| `sys_setting_definitions` | 매장 설정 이름, 값 스키마, 기본값, 판을 기록에 남기는지 | prepayment_mode, same_day_cancel_refund_default, payment_timing_default, rounding_policy, discount_stacking, receipt_number_format, weekday_day_types, walk_in_phone_required, extension_default_return_time, night_collection_notice_minutes, driver_sees_due_amount, lesson_settlement_timing, max_line_quantity, session_idle_minutes, login_lockout, retention, backup_policy, offline_policy, closing_policy; 0002: opening_cash, default_return_slot; 0003(2026-09-26): vehicle_late_after_minutes(`{minutes, night_minutes}`, 기본 60 · 60 — 차량 약속이 늦음이 되기까지의 여유, 야간 반납 타임 뒤는 night_minutes) |
...
| `sys_location_kinds` · `sys_movement_kinds` · `sys_movement_routes` | 재고 위치 종류, 이동 종류, **허용된 (이동, 출발, 도착)** 과 기사 · 오프라인 허용 | 기초 재고, 재고 입고, 발권, 적재, 지급·배달, 차량 수거, 매장 입고, 매장 반납, 발권처 환불, 분실 회수, 거래처 대여 입고 · 대여 출고 · 반납 · 회수, 차량 간 인계, 창고 · 지점 사이 이동(매장 → 매장 포함), 다른 매장 이동 · 다른 매장에서 받음, 폐기·분실, 처리 취소(모든 되돌릴 수 있는 경로의 반대 방향) |
...
| `sys_payment_timings` · `sys_payment_kinds` · `sys_payment_purposes` | 결제 시점, 돈의 종류와 부호 · 필요한 연결(환불 대상, 보증금), 돈의 목적 | at_intake, at_issue, at_return, later, by_other_order, partner_postpaid · payment, refund, legacy_refund(가져오기 전용: 대상 모르는 옛 환불), deposit_in, deposit_out, deposit_apply, deposit_forfeit(`보증금 몰수`: 현금이 움직이지 않음), deposit_restore(`몰수 취소`) · charge, prepayment, deposit |
| `sys_deposit_timings` · `sys_deposit_refund_methods` · `sys_deposit_unreturned_actions` · `sys_deposit_entry_kinds` | 보증금 규칙의 받는 때, 돌려받은 매수의 보증금을 돌려드리는 법(돌려받는 창에서 먼저 골라 둘 것), 안 돌아온 매수의 보증금, 보증금 장부 줄의 종류와 그 줄이 가리켜야 하는 돈 종류 · 매수 부호(4-12) | at_intake(`접수 시`), at_issue(`지급 시`) · cash(`현금 반환`), offset_due(`미수 차감`), same_method(`동일 수단 반환`) · keep(`보증금 몰수`), charge_loss(`분실금 청구(보증금 공제)`, 운영 규칙 화면 두 줄 `분실금 청구` / `보증금 공제`) · take(`보증금 입금`, deposit_in, +1), refund(`보증금 반환`, deposit_out, −1), apply(`미수 차감`, deposit_apply, −1), keep(`보증금 몰수`, deposit_forfeit, −1), restore(`몰수 취소`, deposit_restore, +1) |
...
| `sys_screen_templates` · `sys_column_renderers` · `sys_ledger_filters` · `sys_ledger_metrics` | 화면 틀, 장부 칸 그리기(와 칸이 채워야 할 연결 `binding_key`), 색인 탭 필터, 바닥줄 숫자 | ledger, slip, checklist, collection_list, driver_list, tiles, form, split · time, team, items, promise, place, money, stamp, attribute, text, vehicle, action(전화 · 수거 실패) · all, pickup, return, unpaid, vehicle, lessons, open_tasks, collected, remaining, pinned · team_count, issued_count, returned_count, due_total, collected_count, pending_count, remaining_count, vehicle_load |
| 화면 어휘(동작 이름 `sys_actions.label`은 화면에 나오므로 화면 문구 표([docs/design/wording.md](../design/wording.md))의 말과 같다: `리프트권 추가`, 배달의 `배달 실패` · 수거의 `수거 실패`, 끊긴 카운터의 `권 지급 처리` · `즉시 교환`, 도장은 `지급 처리` · `반납 처리` · `수납 처리`. 보증금 · 반납 정책 어휘의 `label`(사용 내역 · 콘솔 이름)도 같은 말로 맞추고(`보증금 몰수` · `반납 필수` …), 운영 규칙 카드의 고르기 말은 앱의 문구 표에서 온다 — README D1 · D6): `sys_workspaces` · `sys_screens` · `sys_group_keys` · `sys_sort_keys` · `sys_fit_modes` · `sys_fold_modes` · `sys_align_keys` · `sys_overflow_modes` · `sys_tones` · `sys_icons` · `sys_picker_placements` · `sys_conditions` · `sys_status_keys` · `sys_confirm_templates` · `sys_actions` · `sys_input_widgets` · `sys_display_formats` · `sys_row_grains` · `sys_row_grain_entities` | 화면 설정 행이 가리킬 수 있는 코드 키 전부(외래 키 대상) | 작업 공간 · 경로가 있는 화면 · 묶음 · 정렬(시각 여부) · 글자 맞춤 · 합치는 모양 · 정렬 · 넘칠 때 · 색(빨강은 늦음만, 도장 주홍 따로) · 아이콘 · 고르기 자리 · 조건 · 도메인별 상태 값과 기본 문구 · 확인 창 · 동작(명령 + 확인 창 + 되돌리기 묶음) · 입력 방식 · 보이는 모양 · 장부 한 줄의 단위와 붙일 수 있는 속성 주인 |
...
| `sys_offline_limits` | 기기가 오프라인에서 명령을 쌓기 전에 확인하는 제한(셋째 검토, 코드 키가 FK 대상이 되게) | walk_in_cached_quote(`현장 접수 · 기기 게시 요금표 · 사전 설정 할인 · 이 팀 결제 또는 후불`), own_payer_cached_quote(품목 추가: `기기 게시 요금표 · 결제 팀 = 이 팀 · 진행 중 접수`), cached_quote(`기기 게시 요금표`), shop_ticket_stock(`기기 사본 매장 권 재고 · 사용 시간 비중복`), same_line_swap(`같은 품목 줄 번호 · 규격 변경 · 금액 유지`). 도착한 명령이 제한을 벗어났으면 적되 확인 필요 |
...
| 표 | 목적 | 핵심 필드 | 불변식 · 관계 |
|---|---|---|---|
| `payment_methods` | 결제 수단 | `key`, `affects_cash_drawer`, `requires_terminal`, `requires_reference`, `refundable`, `driver_allowed`, `quick`(수단 줄의 버튼), `short_label` | `key` 유일. 새 수단(간편결제, 상품권, 외상 `on_account`)은 행 추가. `(id, affects_cash_drawer)`가 수납의 FK 대상이라 돈이 쓰는 수단의 현금 여부는 못 바꿈. 첫 매장(D7): 빠른 수단 카드 · 현금 · 계좌이체, `driver_allowed`는 현금 · 계좌이체, 지역화폐 · 상품권 행 없음 |
...
| `deposit_rules` | **보증금 규칙**: 종류 전체나 상품 하나가 받는 매당 보증금(예: 리프트권 1매 5,000원. **첫 매장은 행이 없다** — 사장님이 말한 1,000원은 스키장 권 값 안의 카드 보증금이라 손님과 주고받지 않음, README D7) | 대상(종류 또는 상품), `unit_amount`, `timing_key`(`접수 시` · `지급 시`), `payment_section_id`(확정 창의 '리프트권 보증금' 칸), `refund_default_key`(돌려받는 창에서 먼저 골라 둘 것), `unreturned_key`(`보증금 몰수` = keep · `분실금 청구 / 보증금 공제` = charge_loss; 화면 말은 운영 규칙 카드, 둘째 판 시안 V8), `unreturned_after_days`(NULL = 사람이 정함, N = 반납 약속일 N영업일 뒤의 마감이 정함), `loss_amount`, 효력 기간 | 대상은 **정확히 하나**(CHECK). 상품 규칙이 종류 규칙보다 먼저. 행이 없으면 보증금이 없다(다른 매장 · 다른 스키장). 꼭 돌려받는지는 이 표가 아니라 종류의 `return_policy_key`다. 금액을 바꾸면 새 행(효력 날짜)이고, 이미 맡은 보증금은 복사본(`deposits`)대로 돌려드린다. 기능 `deposits`가 꺼지면 규칙은 쓰이지 않음 |
...
| `return_slots` | 반납 타임 | `label`, `local_time`, `day_offset ≥ 0`(상한 없음), `is_night`, 효력 기간 | 야간 수거 알림은 `is_night`. 첫 매장(D7): `야간 22:00`(스키장이 22:00에 끝남)이고 준비 알림은 설정 `night_collection_notice_minutes` 60 → 21:00(차량이 21시부터 올라가 있는 날이 많음). 손님 연락이 몰리는 22:30 ~ 23:00은 같은 22:00 묶음의 수거다: 그 수거가 늦음(빨강)이 되는 때는 설정 `vehicle_late_after_minutes`의 `night_minutes`(첫 매장 90 → 23:30, 마이그레이션 0003; 그 밖의 차량 약속은 `minutes` 60, 매장 반납은 30분 고정). 준비 줄은 21:00부터 23:30 전까지 남는다(2026-09-26 검토) |
...
| `vehicles` · `vehicle_assignments` | 차량과 날짜별 담당 기사 | `name`, `staff_member_id`, `valid_from/to` | 은퇴해도 행 유지. 첫 매장(D7): `1호 차량` · `2호 차량`. 몰리는 날 직원 개인 차는 **임시 차량 행**(차량 + 그 지갑 `cash_drawers` + 담당 기사, 끝나면 `valid_to`)으로 더한다 — 새 표가 아니라 행이고, 더하는 화면(`매장 설정` → `차량 · 직원`)은 아직 없다 |
...
| `card_terminals` · `printers` | 카드 단말, 인쇄 대상 | `vendor_key`, `device_id`, `paper_key` | 비밀값은 DB 밖. 온라인 결제(PG)는 단말 행 없이 카드 요청의 `provider_key`로. 첫 매장(D7): 단말 1대(연동 안 함), 프린터는 A4 하나(라벨 프린터 미정) |
...
| `stamp_steps` · `item_kind_stamp_steps` | 도장 단계(지급 · 반납 · 수납 · 적재 · 발권 · 수거 · 입고 · 강습)와 종류별 순서 · 조건 | `rule_key`, `action_key`(명령 + 확인 창 + 되돌리기 묶음, `sys_actions`), `checklist_label`, `short_label`, 긴급 순위, `shows_time`, `condition_key`(`sys_conditions`) | 새 단계는 행 추가. 새 계산 규칙 · 동작만 코드와 sys 행 필요. 한 종류에 같은 단계가 조건만 다르게 두 번 올 수 있음 |
| `ledger_views` · `ledger_view_columns` · `ledger_view_column_steps` · `ledger_view_tabs` · `ledger_view_metrics` · `ledger_view_primary_actions` · `ledger_view_actions` | 장부 · 접수증 · 수거 목록 · 기사 목록 화면 설정 | 화면 경로, 틀, 기기 등급, 바탕 화면(`base_view_id`), 한 줄의 단위(`row_grain_key`), 묶음 · 보조 묶음, 정렬, 지금 줄, ▲▼, 긴급 먼저 · 칸 최소 · 선호 폭(em), 비율, 빠지는 순서, 합쳐 들어갈 칸과 모양, 도장 모음(`collapse_group_key`), 맞춤 방식, 도장 · 속성 · 동작 연결 · 단계 사슬 · 탭 필터와 값 · 바닥줄 숫자와 값 · 조건별 주 버튼 · 옆 동작 | 모든 코드 키가 sys 어휘 FK. 합쳐 들어갈 칸은 같은 화면의 칸(자기 참조 FK). 목록마다 우선순위 · 넘칠 때(ui 문서 4-4). 기능이 켜졌을 때 또는 꺼졌을 때만 보이기(`feature_on`) |
...
| `order_people` | 일행(사이즈 사전 입력에만 씀) | `seq`, `name`, 연령 구분 | 현장 접수는 대표자만 기록. 부분 결제는 사람을 고르지 않음 |
...
| `line_promises` | 줄(또는 일부 수량)의 수령 · 반납 약속 | 종류, 방법, 수량, 날짜 · 시각, 반납 타임 · 수령 시각 옵션과 스냅샷, 장소 id + 구역 · 장소 이름 스냅샷 + 메모(객실), 차량, 업무, 상태, 앞 약속, **나눈 일정 묶음 `schedule_id`**(0002: 일정 변경으로 옮긴 줄들이 같은 값, 7-4) | 방법은 그 약속 종류의 것만(복합 FK). 줄마다 · 종류마다 살아 있는 약속 수량의 합 = 살아 있는 수량(명령 계층 + 검증기) |
...
| `intake_requests` | 요청 링크 하나(모든 요청은 접수에 딸림, 인원 예약도 접수) | `token_hash` 유일. 매장은 control `public_links`가 먼저 찾아 줌(2-3). 토큰 원문은 URL 조각에만 |
...
| 표 | 목적 | 핵심 필드 | 불변식 · 관계 |
|---|---|---|---|
| `stock_locations` | 재고가 있을 수 있는 모든 곳(매장 · 지점, 창고, 차량, 손님, 거래처, 다른 테넌트, 외부, 폐기·분실) | `kind_key`, 차량 · 거래처 · 접수 연결, `branch_id` | 매장마다 `external`과 `void` 행도 있음. 차량 · 거래처 · 접수마다 하나(부분 유일). `(id, kind_key)` 유일 → 이동의 경로 확인에 씀. 나중의 새 주인 종류(숙소 보관함 등)는 1:1 확장 표(E5)이고 '주인 하나' 규칙은 검증기가 봄 |
| `lots` | 들여온 묶음(구입, 기초, 거래처 대여 입고) | 주인 거래처, `source_reference` | 중복 들이기 방지(부분 유일) |
| `assets` | 번호로 관리하는 장비 하나와 리프트권 한 장(**첫 매장은 없음**: 모든 종류가 수량, README D7) | 상품 · 규격(품목 바꾸기의 투영), 옛 사이즈 글자, 주인, 스티커 번호 + 투영(`location_id`, `condition_id`, `last_movement_id`) | 규격은 그 상품의 것만. 스티커 번호 유일(부분). 위치 · 상태 · 품목은 이동 · 상태 변경 · 품목 바꾸기 기록으로만 바뀜. 지우지 않음 |
...
| `stock_balances` | 수량으로 세는 품목(고글, 보호대; **첫 매장은 모든 대여 종류와 리프트권**, D7)의 위치 · 규격 · 상태 · **주인 · 묶음(lot)**별 수량 | `quantity ≥ 0` | 투영: 수량 이동 줄의 합. 거래처에서 빌린 고글과 우리 고글, 유효기한 있는 핫팩 묶음을 따로 셈 |
...
| `equipment_loans` · `equipment_loan_items` | 거래처 대여 입고 · 대여 출고와 장비 또는 수량(고글 20개)별 나감 · 돌아옴 | 돈은 거래처 장부. 수량은 여러 번에 나눠 돌아올 수 있음(`back_quantity ≤ quantity`) |
...
| `task_visits` | 방문 결과(고객 부재 · 장소 변경 · 물품 미준비, 재방문)(장부) | `result_reason_id`, `retry_key`, 전후 날짜 · 시각 · 장소 | 업무 안 순번 유일. 순번은 서버가 적용할 때 매김(기기가 매기지 않음) |
| `task_reassignments` | 차량 · 날짜 · 시각 옮김(장부) | 전후 값 | 옮기면 옛 차량 범위에 지움 표시, 새 차량 범위에 새 행(sync 문서 5절) |
| `route_positions` | 방문 순서: 업무마다 한 행, 분수 순위 | `rank_key`(BINARY 비교, NULL = 시간순), 차량, 날짜, `decided_at`(고친 발생 시각) | 한 경로의 첫 손 순서 바꾸기가 그 경로 모든 업무에 순위를 한 명령으로 씀. ▲▼는 '이 업무 앞 · 뒤'(기준 업무)로 보내고 서버가 지금 순서에 맞춰 풂. 더 오래된 이동이 늦게 오면 짐(`route_superseded`). 같으면 (순위, 약속 시각, 업무 id). 업무 차량 변경이 같은 트랜잭션에서 차량 · 순위를 다시 씀 |
| `task_pins` | 긴급 요청(화면 `긴급`): 맨 위 고정과 알림 | 메시지, 고정 · 해제, 해제 이유, 알림 | 업무마다 살아 있는 고정 하나(부분 유일). 이미 고정이면 같은 명령은 '이미 됨'(오류 아님). 업무가 끝나거나 취소되면 자동 해제 |
| `notifications` · `notification_receipts` · `notification_preferences` | 직원 · 기사 알림(요청됨 → 기기에 도착 → 사람이 확인 → 닫힘 · 만료), 기기별 도착 · 확인, 소리 설정 | 받는 범위(매장 · 차량 · 직원), 종류, `source_key`, 단계와 시각, 문구 틀 키 · 값 | 출처 키 유일(중복 알림 방지; 긴급 요청은 `pin:<고정 id>`라 다시 고정할 수 있음). 제목에 전화번호 없음. 문구 틀 키와 값(`template_key`, `params_json`)을 함께 두어 다른 언어로 다시 그림 |
...
| `order_checkout_locks` | 접수마다 열린 카드 결제 하나 | 접수, 카드 요청, 기기 | 부분 유일 인덱스. 다른 카운터는 '카드 결제 중 · 카운터 1'을 보고 그사이 그 팀의 카드 · 현금을 받지 못함 |
...
| `payment_allocations` | 한 건을 여러 접수 · 줄에 나눔(장부) | 접수, 줄(NULL = 접수 전체), 금액(수납 이동 안에서만 음수), 수량(부분 결제; 금액과 같은 부호), **결제 칸**(0002의 연결 장부 표 `payment_allocation_sections`: 줄을 고르지 않고 칸을 채운 돈 — 리프트권 선입금, 배분 한 줄에 칸 하나, 7-4), 마감 범위 · `business_date` · `posting_date`, 행위자, 요청번호 | 줄은 그 접수의 것(복합 FK). `수량 × 금액 > 0`(CHECK)이라 되돌린 수량이 '더 낸 수량'으로 세어지지 않음. 품목별로 나눠 내는 접수는 줄 단위 배분만(명령 계층). 배분 합 = 수납 금액(명령 계층 + 검증기) |
| `payment_reallocations` | 수납을 다른 팀 · 줄로 옮김(장부) | 사유 | 옮김 = 원래 쪽 음수 배분 + 새 쪽 양수 배분 |
| `deposits` | 보증금 보관 머리. **규칙 보증금은 팀 · 규칙마다 하나**('이 팀 권 보증금': 어른 권 · 어린이 권 두 줄이어도 하나라 돌려드릴 때 돈 한 건), 규칙 없는 보증금(신분증 대신 현금 등)은 줄이나 장비에 | 규칙과 그 값의 복사본(이름 · 매당 금액 · 돌려드리는 법 · 안 돌아올 때 · 며칠 뒤 · 분실 값), 필요 금액 + 투영(보관 금액 · 보관 매수 · 뺀 매수, 상태) | 돈은 보증금 종류의 수납 행, 매수는 `deposit_entries`. 팀 · 규칙마다 하나(부분 유일). 청구에 자동으로 쓰지 않음(`미수 차감`은 사람이 고르거나 규칙이 먼저 골라 둠). 지우지 않음 |
| `deposit_entries` | **보증금 장부**: 보증금 돈 한 건이 어느 줄의 몇 매인지(장부) | 종류(받음 · 돌려드림 · 미수에서 뺌 · 보증금에서 뺌 · 되돌림 — 화면 이름 `보증금 입금` · `보증금 반환` · `미수 차감` · `보증금 몰수` · `몰수 취소`)와 그 종류가 가리켜야 하는 돈 종류(복사), 줄, 권, 매수, 금액, 수납, 돌려받은(또는 폐기한) 이동 줄, 마감, 사유, 네 시간 | 보관 매수 = 받음 + 되돌림 − 돌려드림 − 미수에서 뺌 − 보증금에서 뺌. 줄은 그 팀의 것, 돈은 **같은 보증금 · 맞는 종류**의 수납(`(수납, 종류, 보증금)` 복합 FK), 돌려드림의 이동 줄은 **그 품목 줄의 권을 옮긴 줄**(`(이동, 줄 번호, 품목 줄)` 복합 FK). 돈 한 건이 여러 줄을 가짐(2매 한 번에 10,000원 = 수납 하나 + 줄 둘) |
...
| `closings` | 마감 한 판: 범위, 시재, 예상 · 실제 현금, 차액, `as_of_rev`, 확인한 미리 보기(`basis_json`), 넘어가기 사유(`override_reason`), 얼린 보고서(장부) | `차액 = 실제 − 예상`. `(범위, 날짜, 판)` 유일. 보고서에 이름 없음(id와 숫자만) |
| `closing_reopenings` | 마감 다시 열기(관리자, 사유)(장부) | 한 판은 한 번만 다시 엶. 그 범위의 가장 늦게 닫힌 날만(3-3) |
| `closing_totals` · `closing_drawer_counts` · `closing_handover_items` | 마감 숫자(수단 · 결제 칸 · 품목 종류 · 거래처 · 세금 분류별), 돈통 · 차량 지갑별 실제 금액(앞 셈 · 앞 `as_of_rev`, 셈 상태 counted · deferred), 다음 날로 넘기는 일(미수, 과수납, 보증금, 미반납, 늦게 들어온 돈, 확인 필요, **열린 대신 내기 약속**, 미확인 현금 인계, 결과 모르는 카드, 발권처 환불 대기)(장부) | 돈통별 `차액 = 실제 − 예상`. 인계 항목은 접수가 없어도 됨(대상 종류 + id) |

마감 절차: 범위 · 날짜의 `business_days` 행을 잠그고 → 청구(차수)와 돈 장부를 `closing_scope_id = 그 범위`, `posting_date = 그날`, `created_rev ≤ as_of_rev`로 합산 → **돈통마다** 예상 현금 = 그 돈통의 앞 셈(`prev_closing_id`의 센 금액) + `cash_movements.created_rev`가 `(앞 as_of_rev, as_of_rev]`인 합(날짜가 아니라 rev 창이라 늦게 들어온 차량 현금이 두 번 세어지거나 빠지지 않음) → `closings` 판 추가 → `business_days.active_closing_id` 설정. 마감 미리 보기는 **오프라인에서 기록할 수 있는 모든 기기**(카운터 · 기사 기기)마다 마지막 동기화 시각과 보낼 줄 수를 보이고(`device_sync_cursors`), 그 마감 범위(`devices.closing_scope_id`)의 기기 중 미리 보기 `as_of` 뒤로 맞추지 않았거나 보낼 것이 남은 기기가 있거나, 그날 마지막 연락 뒤로 소식이 없는 카운터가 있거나, 돈 검증 결과가 열려 있으면 막는다(`closing_policy.block_on_unsent_devices` · `block_on_unsent_van`; 확인 필요 차량 `van_unsynced` · 카운터와 그 밖 `device_unsynced`, `카운터 2 · 마지막 연결 16:10 · 전송 대기 있음`). 관리자가 **비밀번호(또는 휴대폰 확인)와 사유로** 넘길 수 있고, 그러면 그 기기가 넣는 돈통(차량 지갑 · 그 카운터의 돈통)의 셈은 `deferred`로 두고 다음 셈에 넘긴다: 카운터 2의 현금이 빠진 채 예상 현금이 계산되어 틀린 차액이 얼지 않게. 보낼 기록이 남은 기기가 있으면 **그 마감은 보증금 자동 정리(`unreturned_after_days`)를 건너뛴다**(그 기기 큐 안에 돌아온 권이 있을 수 있다). 끊긴 동안 나간 현금(보증금 돌려드림 · 보증금 더 돌려드림)은 미리 보기에 `연결 끊김 중 현금 출금 · 담당자`로 따로 보이고 관리자 확인을 받는다(sync 7절). 다시 열기: `closing_reopenings` 추가 → `active_closing_id = NULL`. 새로 마감하면 판 번호가 하나 오른다. 마감된 날 뒤에 들어온 돈 · 청구는 3-3의 기준선으로 `posting_date`를 받는다.

**마감이 보이는 하루와 보증금(2026-09-24):**
- 마감 화면의 제목줄 오른쪽은 기준 시각 띠다: `현재 27일 00:40` · `26일 장부 · 기준 06:00 · 00:15 정하늘 · 0041 반납 포함`(3-3, 둘째 판 시안 V6). 자정 뒤에 마감해도 그날 장부를 닫고, 새벽 6시 전에 마감한 뒤 들어온 반납 · 돈은 다음 열린 날(27일) 마감이 센다.
- **마감 차례(V6):** 주황 주 버튼은 늘 지금 할 일 하나다: `차량 현금 점검 · 35,000원` → `돈통 점검` → `마감`. 차량 현금을 먼저 세는 까닭: 세고 확인해야 넘기는 중 돈통에서 카운터 돈통으로 들어오므로(4-12) 돈통을 먼저 세면 그 셈이 틀린다. 그래서 화면의 카운터 돈통 예상은 차량 현금 확인 뒤의 값이다(S16: 510,000 + 35,000 = 545,000원, `보증금 5,000원 포함`). `점검 이월`을 고르면 차량 봉투는 `미확인 현금 인계`로 넘어간다. 마감은 연결이 있어야 한다(모든 기기의 기록이 모여야 함, sync 8-2).
- **보증금은 매출이 아니다.** `결제 수단` 표(수단별 수납)에 넣지 않고, 현금으로 받거나 돌려준 보증금은 `cash_movements`에 있으므로 돈통 예상 현금에만 들어간다(카운터 돈통 예상 = 시재 + 현금 수납 + 보증금 받음 − 보증금 돌려드림). 차량 지갑에서 돌려드린 보증금은 그 지갑의 셈에 든다.
- `closing_totals`에 `deposit_in` · `deposit_out` · `deposit_apply` · `deposit_forfeit` · `deposit_restore`(그날 기준일의 돈)와 날 끝의 `deposit_held`(보관 중 금액)를 얼린다. 차원은 `deposit_rule:<id>`, 매수는 `row_count`에 보증금 장부의 매수를 넣는다. 화면 줄: `보증금 보관 중 · 리프트권 1매 · 5,000원` / `최하은 · 0026 · 돈통 보관`.
- 아직 맡고 있는 보증금은 팀마다 인계 항목(`issue_key 'deposit'`, `deposit_held_amount`, `open_quantity` = 안 돌아온 매수)으로 넘어가고, 반납 약속이 지난 권은 `return` 인계에 늦음으로 오른다(`리프트권 미반납 · 1매 · 지연`, 빨강은 이 지연에만).
- 보증금 규칙의 `unreturned_after_days`가 찬 매수는 마감 미리 보기에 한 줄로 보이고(`최하은 · 0026 권 1매 보증금 5,000원 · 마감 시 몰수` — 운영 규칙의 `보증금 몰수`와 같은 말), 마감 명령이 같은 트랜잭션에서 폐기 이동(손님 → 폐기·분실, 줄의 `qty_not_returned`) · `deposit_forfeit` 수납 · 보증금 장부 `keep` 줄(`closing_id`)을 쓴다. 그 돈의 날짜는 그 마감의 날이다. `unreturned_after_days`가 NULL이면 사람이 접수증이나 마감 인계에서 `보증금 몰수`(`deposit.keep`; 버튼 · 규칙 · 어휘가 한 말, README 열린 질문 25 · D6)를 누를 때까지 보관 중으로 넘어간다. `charge_loss` 규칙이면 보증금에서 빼는 대신 분실 조정(`loss_amount` × 매수, `adjustment_assets`)을 청구하고 보증금으로 먼저 낸다(`deposit_apply`).
- **보증금 규칙이 없는 매장(첫 매장, 2026-09-26 README D7):** 위의 보증금 줄이 모두 없다: 돈통 예상에 보증금이 없고, `closing_totals`에 `deposit_*`가 없고, 이월 항목에 `보증금 보관 중`이 없다. 반납 일정이 지난 권은 `return` 인계의 `리프트권 미반납 · N매 · 지연`(빨강)으로만 넘어가고 손님에게 청구하지 않는다. 그 줄을 청구 없이 닫는 법(사람이 정리 · 마감이 N일 뒤 정리 · 그대로 둠)은 README 열린 질문 35이고, 정리한다면 폐기 이동(손님 → 폐기·분실, 줄의 `qty_not_returned`)만 적고 돈 · 조정은 없다.

### 4-14. 거래처 장부

| 표 | 목적 | 불변식 · 관계 |
|---|---|---|
| `counterparty_trades` · `counterparty_trade_lines` | 미수 · 미지급(권 구입 · 판매 · 환불, 장비 대여 비용 · 대여 수입, 강습 위탁, 수수료, 후불 거래처 청구)과 내역(장부) | 방향은 종류가 정함. 정정은 취소 기록(`cancels_trade_id`, 한 번만). 만든 근거는 `source_key`로 한 번만(강습 예약을 두 번 닫아도 두 번 빚지지 않음). 후불은 접수를 비운 비현금 수납(`payment_id`)과 연결 |
...
| `outbox` · `outbox_attempts` | 커밋 뒤 실행할 일(문자, 인쇄, 카드 단말, 외부 알림, 백업 전송)과 시도 | `(채널, 멱등 키)` 유일, 멱등 키는 만든 요청에서 정해짐. 자동 재시도는 채널 값을 복사(문자 · 카드는 0). 잡기(`in_flight`)는 바깥 호출 전 따로 커밋. 복구 뒤 다시 만든 행(`replayed`)의 재시도 없는 채널은 `unknown`에서 시작. 결과는 정해진 요청번호의 시스템 명령으로 기록 |
...
| `device_sync_cursors` | 기기별(카운터 · 기사) 마지막 rev, 기기 번호, 시계 차이, 대기 수, 앱 판 · 스키마 판, 마지막 받기 · 보내기 | 마감 미리 보기(모든 기기: '1호 차량 · 마지막 연결 21:40 · 전송 대기 있음', '카운터 2 · 마지막 연결 16:10 · 전송 대기 있음')와 관리자 콘솔의 기기 목록 · 감시(보냄 대기가 줄지 않음, 옛 판)가 읽음. 마지막 받기가 기기 라이선스 표를 마지막으로 준 때다. 서버는 스키마 판에 맞춰 모르는 표 · 열을 빼고 보낸다(sync 5절) |
...
사장님 질문(2026-09-24): 일괄 반납 말고 부분 반납 같은 것도 나중에 더할 수 있나? **된다. 더할 표도, 다시 만들 표도 없다.** 반납 · 약속 · 돈 · 보증금이 처음부터 **품목 줄과 수량** 단위이고 사람 단위가 아니기 때문이다. 현장 접수는 대표자 이름만 적으므로(사장님 확인 2026-09-21) 나누는 단위가 사람일 수 없다: `order_lines.person_id`와 `order_people`은 사이즈 사전 입력에만 쓰고 반납 · 수납 · 보증금을 나누지 않는다. 이번에 같은 접수를 데이터베이스가 보증하지 않던 두 곳(조기 반납 · 교환의 줄과 약속)을 복합 FK로 채웠다(4-10).
...
| 경우 | 무엇이 적히나 | 남는 것 |
|---|---|---|
| **일괄 반납**(반납 창을 열면 모두 골라져 있으므로 그대로 누름: `반납 처리 · 6개 · 3매 · 보증금 15,000원`) | 반납 이동 하나에 장비 · 권마다 이동 줄, 보증금 돈 한 건(3매 15,000원)과 권마다 돌려드림 줄. 명령은 반납(`stock.direct_return`, 본문 `lines: [{ lineId, quantity, assetIds }]` — 번호로 세는 줄은 돌아온 번호) 하나와 보증금 명령(`deposit.return`, `dependsOn`) | 없음. 반납 약속은 `fulfilled`, 그 약속의 차량 업무는 받을 것이 없어 수거 목록에서 빠진다 |
| **일부는 지금, 나머지는 나중에 다른 곳 · 다른 시각**(박준호 팀, S16: 21:30 매장에 스키 2 · 헬멧 2 · 권 2매, 22:00 솔마을 두솔동에서 보드 1 · 헬멧 1 · 권 1매) | 21:30 매장 반납 이동(가져온 것만)과 권 2매 보증금 10,000원 돌려드림(또는 `미수 차감`). 약속은 고치지 않는다: 가져온 수량은 그 줄의 반납 수량이 되고, 그 약속의 차량 업무는 `task_items.elsewhere_quantity`로 `매장 반납`이 되어 남은 것만 받는다(차량 기록이 뒤늦게 오면 `already_returned`). 매장에서 먼저 받은 수량은 **따로 나눠 옮긴 약속을 남기고 원래 약속부터** 채운다(손님이 두솔동에서 주기로 한 것은 그대로). 나머지를 다른 곳에서 받기로 한 것은 그 전의 `일정 변경`(수량 일부, 19:40)이 약속을 둘로 나눈 것이다 | 보드 1 · 헬멧 1 · 권 1매와 그 약속(22:00 두솔동 · 1호 차량), 권 1매 보증금 5,000원 보관 |
| **리프트권만 먼저 반납**(권은 16:00에 다 썼고 장비는 밤에) | 권 줄의 이동 줄만(`direct_return` 또는 차량 `collect`)과 그 매수의 보증금 돌려드림. 권 줄은 반납이 끝나 줄 끝 | 장비 줄과 그 약속 그대로. 접수는 `partial_return`(`부분 반납`) |
...
| **안 돌아온 권**(최하은 팀 1매) | 마감 인계에 `리프트권 미반납 · 1매 · 지연`과 보증금 보관 중. 규칙대로(이 매장: 반납 약속일 다음 영업일의 마감, 4-13) 또는 사람이 `보증금 몰수`: 폐기 이동(손님 → 폐기·분실, `qty_not_returned`) + `deposit_forfeit` 한 건. 나중에 돌아오면 분실 회수 이동 + `deposit_restore` + 돌려드림(확인 필요 `deposit_kept_returned`) | 없음(줄 끝) |
...
- 새로 만들 것은 **화면**뿐이다: 반납 창의 품목 칸(번호 · 수량, 처음에 모두 골라 둠)과 `미반납` · 보증금 줄 · `반환 방법`(`현금` · `미수 차감 · 120,000원 → 110,000원`), 일정 변경의 품목 수량(ui 6-2, 채택한 둘째 판 시안 `docs/design/screens-v2/spec.md`의 V1 · V9).
- **첫 매장(2026-09-26, README D7):** 모든 종류가 수량이라 반납 창의 품목 칸이 모두 −/+이고(처음 값 = 이 팀이 가진 수), 이동 줄은 규격 + 수량이다(`assetIds` 없음). 보증금 규칙이 없어 위 표의 보증금 줄(돌려드림 · `미수 차감` · 몰수)과 `deposit.return` 명령이 없다: 일괄 반납은 `반납 처리 · 6개 · 3매`, 리프트권만 먼저 반납은 권 줄의 수량 이동 하나, 안 돌아온 권은 청구 없이 남는다(4-13). 끊긴 카운터에서 권을 처음에 고르지 않는 규칙(보증금이 두 번 나가지 않게)은 보증금이 없으므로 첫 매장에는 걸리지 않고, 카운터도 한 곳이다.
...
- 업무마다 살아 있는 긴급 요청 하나. 알림 출처 키 유일. 접수마다 열린 카드 결제 하나.
...
- 보증금(2026-09-24): 보증금 규칙의 대상은 하나(종류 또는 상품), 팀 · 규칙마다 보관 하나. 보증금 장부 줄은 그 팀의 보증금 · 그 팀의 줄, 줄의 종류와 돈 종류가 맞고 그 돈은 **같은 보증금**의 수납, 돌려드림 · 폐기의 이동 줄은 **그 품목 줄을 옮긴 줄**. `보증금 몰수` · `몰수 취소` · `미수 차감`은 돈통이 없다(현금이 움직이지 않음, 종류의 `cash_sign` 0).
- 조기 반납 · 교환 단위의 줄 · 약속은 같은 접수의 것.
- 접수 번호, 카드 요청당 같은 승인번호 한 건, 멱등 키(요청번호), 기기 번호, 작업 기록 요청번호, outbox 멱등 키, 재고 입고 출처, 스티커 번호, 토큰 해시 유일. (카드 승인번호 자체는 유일하지 않다.)
- 부호 · 범위(금액 > 0, 수량 ≥ 1, 0 ≤ 취소 ≤ 수량, 비율 0~10000), `net = gross − discount`, `공급가 + 부가세 = net`, `차액 = 실제 − 예상`, `posting_date ≥ business_date`, `valid_to > valid_from`, 요금 규칙 대상 하나.
...
- 보증금 매수: 돌려드림 · 미수에서 뺌 · 보증금에서 뺌의 매수 ≤ 그 팀 보관 매수. 돌려드림은 돌아온 권(그 이동 줄)이나 지급 전에 취소한 매수만, 보증금에서 뺌은 반납 약속이 지났는데 돌아오지 않은 매수만(폐기 이동과 같은 트랜잭션), 되돌림은 뺀 매수 안에서. 받는 때가 `접수 시`인 규칙인데 접수 때 받지 못한 팀(전화 예약)은 지급 창이 보증금을 먼저 묻는다. 규칙은 줄의 이용 시작일에 효력 있는 것(상품 규칙 > 종류 규칙)을 쓰고 보관에 값을 복사한다.
...
- 권한 · 범위 · 한도, 기능 켜짐, 기사의 차량 범위(**일이 일어난 시각의** 업무 배정으로), 가격 = 서버 견적(오프라인 사실은 그때 효력 있던 판의 기기 견적) 또는 권한 있는 직접 입력, 할인 중복 규칙.
...
- `service`: 예약이 모두 끝 · 취소 · 불참일 때. `fee`: 만들 때. `bundle`: 자식 줄이 모두 끝날 때. `placeholder`(인원 예약): 끝나지 않음 — 당일 품목으로 바꾸거나 취소해야 한다.

**수납 상태(`pay_state_key`)**: `paid`는 미수 0이고 **이 팀이 대신 내기로 한 금액도 0**일 때만이다. 다른 팀이 내기로 한 금액만 남았으면 `promised`(도장은 '예정', 끝이 아님), 이 팀이 대신 낼 금액이 남았으면 수납 창과 접수증에 '김OO 팀분 대납 120,000원'이 보이고 끝이 아니다. 어느 쪽 접수든 닫히거나 그날이 끝날 때 열린 약속은 마감 인계(`promise_open`)에 오른다.

**접수 상태**는 옛 순서를 지키고 두 가지를 더했다: `cancelled`(살아 있는 줄 없음) → `awaiting_exchange` → `needs_review`(위치 모름) → `partial_return` → `in_use` → `awaiting_shop`(`입고 대기`) → **`awaiting_load`**(차량 배달 약속이 있는데 아직 적재 전, N1) → `awaiting_issue` → **`booked`**(강습 · 인원만 남음) → `returned` 또는 **`completed`**(보관 책임 줄이 없는 접수). 강습 줄은 예약이 닫히면 끝나므로 접수를 영원히 열어 두지 않는다. 미수는 접수를 열어 두지 않는다(수납 상태와 마감 인계에서 보인다).
...
| E6 | 판 번호가 붙은 설정(`shop_settings`, 값 스키마로 검증) | 매장 · 지점 정책(선입금 방식, 끝전, 할인 중복, 결제 시점, 마감 확인) | 새 판 INSERT |
...
- 시작할 때 데이터베이스에 이 앱이 모르는 마이그레이션이 있으면(앱을 옛 판으로 되돌림) **읽기 전용으로 시작**하고 '읽기 전용 · 새 버전 적용 필요'를 보인다. 모든 요청이 500이 되는 옛 동작을 없앤다. 그래서 **마이그레이션이 든 배포는 앱을 되돌리지 않는다**: 스키마 배포(새 열 · 표를 더하고 그것을 쓰는 기능은 꺼 둠)와 그것을 쓰는 코드 배포를 나누고, 되돌리기는 코드 배포만 한다(deployment 7절). 모르는 판이라도 '뒤로 호환'이라고 표시된 마이그레이션은 읽기 · 쓰기로 여는 방식(예: `compatible_from_app`)은 여러 판이 함께 도는 때에 1주차 실행기 마무리와 함께 정한다.
...
### 7-4. 0002 첫 서버 연결(2026-09-26)

0001을 운영 파일에 적용한 뒤의 첫 더하기 마이그레이션이다(`0002_shop_server_link.sql`, ADR-20, work/impl-server/plan.md §4-3). 서버 저장소(`packages/store`)가 도메인 상태(`ShopState`)를 표로 옮기고 되읽을 때(`load(write(x)) ≡ x`) 0001에 자리가 없던 네 가지만 더했다. 나머지는 모두 0001 표에 맞춘다.

- `line_promises.schedule_id`: 일정 변경(N2)으로 줄 수량 일부를 옮긴 일정 하나의 id. 같은 일정으로 옮긴 줄들이 같은 값을 갖고 차량 업무 하나가 된다. 부분 인덱스 `line_promises_schedule`. 원래 일정 행은 NULL이다. 원래 일정의 남은 수량이 0이 되면 그 행은 superseded만 되고 새 행은 없다(`CHECK (quantity >= 1)`).
- `payment_allocation_sections`(새 연결 장부 표, E5): 줄을 고르지 않고 결제 칸을 채운 배분(전화 예약의 리프트권 선입금). 배분 한 줄(`payment_id`, `seq`)에 칸 하나, 배분 · 결제 칸에 복합 FK, 추가 전용 트리거(고치기 · 지우기 금지). 장부 표 `payment_allocations`에 열을 더하지 않은 까닭: 더한 열의 '한 번만' 트리거는 빈 칸 → 값 한 번을 허락해, 이미 적은(마감한 날의) 배분이 나중에 칸을 얻을 수 있다.
- `line_promise_fulfillments`(새 장부 표, E5): 이동 줄 하나가 어느 일정 몫을 채웠나(나눈 일정으로 들어간 매장 반납, 나눈 일정 업무의 수거 · 입고). `(이동, 줄 번호)`와 `(접수, 약속)` 복합 FK, 수량 ≥ 1, 추가 전용 트리거.
- 설정 둘(`sys_setting_definitions`): `opening_cash`(매장의 **첫** 마감의 시재만. 뒤의 마감은 앞 마감의 카운터 돈통 센 금액을 잇는다, `effective_dated 0`)와 `default_return_slot`(권이 없는 새 접수의 반납 시각 처음 값. `return_slots.sort`는 고르는 차례라 기본값을 싣지 못한다). `added_in`은 들어온 마이그레이션 번호 2다.

`schema.sql`은 0001의 참조 DDL이라 고치지 않는다. 시험은 `packages/schema/test/migration-0002.test.js`(0001 행이 든 파일에 적용 · 네 가지 저마다).
...
**S2 새 물리 품목과 규격.** 고글(수량): `item_kinds('goggles', tracking count)`, 상품 하나, 규격 성인 · 어린이, 재고는 `stock_balances`, 이동 줄은 규격과 수량 3. 보호대: 선택 속성 `protector_size`(S · M · L)를 규격 축으로. 어린이 스키: 장비 종류의 상품, 규격 축 `ski_length_cm`, 장비마다 `assets` 행, 요금은 `customer_class = child`. 프리미엄 보드: 상품 + 자기 요금 규칙. 구성품 부츠: 상품 '스키 부츠'(세트로만 팔면 `hidden_in_picker`)와 `catalog_item_components`(스키 세트 = 스키 1 tracked + 부츠 1 tracked + 폴 1 implied). 접수에는 값이 있는 세트 줄(`bundle`)과 0원 자식 줄이 생겨 부츠만 따로 교환 · 반납된다. 종류별 사이즈 칸은 `item_kind_attributes`, 질문 틀, 인쇄 칸. 수량 품목도 교환(`exchange_units`의 규격 · 수량: 고글 M → L 2개), 조기 반납(6개 중 2개), 거래처 대여 입고(20개, 여러 번에 나눠 거래처 반납), 준비(수량 잡기)가 된다. 여섯 대의 보드를 새 '프리미엄 보드' 상품으로 올리거나, 어른 스키로 잘못 넣은 어린이 스키를 고치거나, 같은 상품 둘을 합칠 때는 `asset_reclassifications` 한 행씩과 `assets.catalog_item_id` 투영 — 지난 이동 줄은 그때 상품을 스냅샷으로 가지고 있어 건드리지 않는다. 보호대(무릎 + 손목)처럼 값이 여러 개인 칸은 `multi_valued` 속성. DDL 변경 없음.
...
**S5 카운터 2대, 차량 3대, 휴대폰 기사가 동시에.** 매장 전체 버전이 없다. 부딪힘은 같은 결정을 두 사람이 바꿀 때, 또는 **결정의 바탕이 된 돈 · 수량이 그사이 바뀌었을 때**만 생긴다. 카운터 1의 수납과 카운터 2의 반납 약속 변경은 서로 다른 행이라 둘 다 된다. 같은 줄의 반납 약속을 둘이 바꾸면 두 번째만 충돌하고 누가 무엇으로 바꿨는지 보인다. 카운터 2가 '수납 0 · 환불 없음'을 보고 취소를 누르는 사이 카운터 1이 그 팀 돈을 받았으면 취소는 이어 붙지 않고 '이 팀 120,000원 수납 완료 · 다른 카운터'가 뜬다(읽은 키 `order:<id>:money`). 두 카운터가 한 팀을 동시에 카드로 받으려 하면 두 번째는 '카드 결제 중 · 카운터 1'을 본다(`order_checkout_locks`). 긴급 요청과 기사의 ▼는 서로 다른 행(`task_pins`, `route_positions`)이라 둘 다 된다. 같은 장비를 두 기사가 잡는 계획은 부분 유일 인덱스가 두 번째를 거절하지만, 실제로 옮긴 사실은 늘 기록되고 그 계획이 풀린다. 쓰기는 파일마다 하나씩 처리되고 한 건에 약 0.6 ms다. 두 번째 가게(지점)는 자기 마감 범위로 따로 닫는다. sync 문서 4 · 8절.

**S6 밤 두 시간 오프라인 차량.** 기사 기기는 허용된 명령만 큐에 쌓고(수거, 차량 배달, 방문 결과, 현장 수납, 권 추가, 순서 변경, 알림 확인), 각 명령에 그때 로그인한 기사를 기기 키로 서명해 둔다. 기기는 같은 도메인 코드를 자기 범위 자료 위에서 돌려 목록 · 도장 · '완료 4 · 전송 대기 8 · 잔여 3'을 그린다(앱을 껐다 켜도). 다시 연결되면 기기 번호 순서로 보낸다(순서는 강제가 아니라 빈 번호만 찾음). 명령마다 따로 적용되고 요청번호로 한 번만 들어간다. 이미 매장에서 받은 장비는 `superseded`, 남은 수보다 많이 받은 경우는 맞는 만큼만 적용하고 나머지는 `확인 필요`, 그사이 카운터가 다른 차량으로 옮긴 업무라도 **일이 일어난 시각에 이 차량 업무였으면** 적용하고 옮겨 간 업무를 취소한다. 돈은 절대 멈추지 않고 기록하며 마감된 날이면 기준선의 `posting_date`를 받는다. 차량 기록이 아직 오지 않았으면 마감이 먼저 알려 준다. sync 문서 8절에 21:40~23:40 예시가 있다. 가게 인터넷이 끊겨 **카운터**가 오프라인인 경우(지급 · 반납 · 적재 · 권 지급 · 즉시 교환 · 수납 · 보증금 · 임시 접수 번호의 새 접수, 두 카운터의 겹침, 끊긴 동안 서버 되살리기)는 sync 8-6 ~ 8-12(S18)이다. 더한 표는 `sys_offline_commands` · `sys_offline_limits`와 두 열(`orders.provisional_receipt_no`, `devices.short_no`)뿐이다. 도착한 큐 명령은 적어 두는 사실이다(sync 8-12).

**S7 여섯 가족 한 번에, 한 팀 나눠서.** 카드 한 번 480,000원: `payment_groups(multi_order)`, `payments` 한 행(승인번호 하나), 접수 · 줄마다 `payment_allocations`. 여섯 팀 미수가 모두 정확히 0이 되고 카드 전표와 기록이 한 건으로 맞는다. 내기 전에는 각 팀에 `payment_promises(by_other_order, payer_order_id)`가 있어 빨간 미수 대신 '김OO 팀 결제 예정'(파란 '예정' 도장)이 보이고, **내기로 한 김OO 팀 쪽에는 '대납 400,000원'**(`collect_for_others_amount`)이 보여 그 팀이 자기 몫만 내고 가지 않는다. 한 팀 나눠서(S08R): `payment_groups(split_by_items)`, 회차마다 수납 하나와 고른 줄의 배분(`allocated_quantity`, 금액과 같은 부호). 그 전에 접수 전체로 받은 선입금은 먼저 줄로 옮겨 나눈다(명확한 '어느 품목이 낸 것인가'). 똑같이 나누기는 비율 배분. 한 가족 몫 환불: `refund` 수납(`refund_of_payment_id`)을 그 접수에만 배분.

**S8 선입금 방식과 당일 취소.** `shop_settings.prepayment_mode`: 이 매장 `{"mode":"full_lift_ticket"}`, 다른 매장 `{"mode":"fixed_amount","amount":50000}` 또는 `{"mode":"none"}`. 전화 예약은 결제 약속을 만든다(리프트권 칸 `prepayment` · `at_intake` · 권 합계, 장비 칸 `at_issue`). 받은 돈은 `payments.purpose_key = 'prepayment'`로 권 줄에 배분 → 카드에 '예약금 N 수납 · 미수 M'. 당일 취소: `order_cancellations`(환불 결정 기본값은 `same_day_cancel_refund_default`), 취소 줄, 음수 `cancellation` 조정. 환불하면 선입금에 연결된 `refund` 수납으로 미수 · 과수납이 0. 환불하지 않으면 양수 `cancellation_fee` 조정이 받은 돈을 청구로 남긴다. 사 둔 권은 장비로 남고 claim은 `cancelled`로 끝나며, 다음 손님 재사용이나 발권처 환불은 따로 한다. **첫 매장(2026-09-26, README D7):** 기본은 `환불`(`same_day_cancel_refund_default = refund`)이고, 너무 늦게 연락한 당일 취소는 직원이 취소 창에서 `환불 없음`으로 바꾼다. 노쇼(연락 없이 안 옴)도 같은 취소 · 같은 환불 결정이다: 거의 돌려주지만 그날 잠수 타고 다음 날 온 손님은 `환불 없음`. 노쇼만 가르는 설정은 없다. 권은 수량이라 취소한 권은 확보(`stock_holds`)가 풀리고 매장 권 재고에 그대로 있다.

**S9 기사 현장 수납과 차량 예비권 추가.** 기사 역할에 `payment.collect_field`(범위 own_vehicle), 수단은 `driver_allowed`인 것만(첫 매장은 현금 · 계좌이체, 안 되면 `후불 처리`로 매장에서 받음 — README D7). 현장 수납은 `collected_by_vehicle_id`, `cash_drawer_id` = 그 차량 지갑, 업무의 접수에 배분, 행위자는 기기가 서명한 그 기사. `후불 처리`는 `payment_promises(later)`. 권 추가는 명령 하나 `field.add_ticket`(자기 업무만, `ticketed` 종류, 그 차량 위치의 장비): 차수(`source_key driver_field`, '배달 중 추가', 자기 `posting_date`), 권 줄(가격은 그때 효력 있던 요금표 판의 기기 견적, `price_source_key 'offline_quote'`), 시간대 claim, 차량 → 손님 `deliver` 이동, 선택한 수납을 한 트랜잭션에. 권을 줄 수 없으면 줄 · 이동은 확인 필요, 수납은 접수 전체 배분으로 그대로 기록. 저녁에 `cash_transfers`(차량 지갑 → 넘기는 중)와 카운터가 세어 확인(넘기는 중 → 카운터 돈통, 차액 → 과부족), 마감은 돈통 · 지갑마다 rev 창으로 센다.
...
**S15 인쇄.** `print_templates`와 판(A4 한 장 20명 준비표, 80×50 팀 스티커, A4 수거 목록, 대여 접수증). 틀은 화면과 **같은 설정**(`source_view_id`의 인쇄 등급 장부 칸, 입력폼 · 문서 자리)을 인쇄 크기(A4 약 794px, 80 mm 영수증, 80×50 스티커)로 풀어 쓰고, 작업을 만들 때 풀어 쓴 설정이 지금 판과 다르면 새 판을 먼저 쓴다. 문서는 중앙 서버가 한 번 그리고(끊긴 카운터는 같은 코드로 기기에서) 도우미나 브라우저는 내보내기만 한다. `print_jobs`에 얼린 문서, 기준 rev, 근거 행과 판(`print_job_sources`), 시도. `print` 채널의 outbox로 인쇄 도우미나 브라우저에 보내고, 도우미는 인쇄 작업 id로 중복을 걸러 응답을 잃어도 두 번 찍지 않는다. 다시 인쇄는 문서를 복사한다. 문서 해시로 '출력 후 변경됨'을 알 수 있다. 인쇄 창을 여는 것만으로 무엇도 처리됨으로 바뀌지 않는다. 첫 매장(2026-09-26, README D7)은 A4 프린터 하나라 A4 틀(수거 목록 · 준비표)을 먼저 쓰고, 80×50 팀 스티커는 라벨 프린터를 살지 정할 때까지, 80 mm 영수증 틀은 영수증 프린터가 없으면 쓰지 않는다(README 열린 질문 36).

**S16 리프트권 보증금과 부분 반납(사장님 답 2026-09-24).** *첫 매장은 2026-09-26 README D7로 이 시나리오의 보증금이 없다(권 값 안의 스키장 카드 보증금이었음) — 아래는 손님 보증금 규칙을 쓰는 매장의 시나리오로 남기고, 첫 매장이면 보증금 돈 · 장부 줄 · 몰수가 빠지고 번호(`5번` · `33번`) 대신 수량을 −/+로 뺀다(4-18).* 설정: 종류 `lift_ticket`의 `return_policy_key = required`(이 매장, 관리 → 매장 설정 → 운영 규칙에서 고름), `deposit_rules('lift_ticket_card', 대상 종류 lift_ticket, 1매 5,000원, 접수 시, 결제 칸 lift_deposit, 현금 반환, 보증금 몰수(keep), unreturned_after_days 1)`, `shop_features('deposits')` 켬. 다른 매장은 규칙 행이 없으면 보증금이 없고, `optional`을 고르면 권 줄이 지급으로 끝난다. 하루(docs/design/screens-v2 명세 2-4의 뒷이야기와 같은 숫자):
...
- 19:40 `일정 변경`(수량 일부): 보드 1 · 헬멧 1 · 권 1매만 22:00 솔마을 두솔동 · 1호 차량으로. 약속이 둘이 된다(설천 주차장 · 두솔동).
- 21:30 가족이 매장에 스키 2 · 헬멧 2 · 권 2매를 가져옴: 반납 창(V1)은 처음에 모두 골라져 있고 직원이 안 가져온 `5번`(보드) · `15번`(헬멧) · `33번`(권)을 눌러 뺀다. `stock.direct_return` 하나(본문 `lines: [{ lineId, quantity, assetIds }]`, 돌아온 장비 · 권마다 이동 줄)와 `deposit.return`(`dependsOn`, 주 버튼 `반납 처리 · 스키 2 · 헬멧 2 · 권 2매 · 보증금 10,000원`): `deposit_out` 현금 10,000 + 돌려드림 줄 둘(권마다 그 이동 줄). `미수 차감`을 고르면 대신 `deposit_apply` 10,000 + 장비 줄 배분이고(미수 120,000 → 110,000), 돈통은 움직이지 않는다. 설천 주차장 업무는 받을 것이 없어져 수거 목록에서 빠지고 두솔동 업무는 남는다(4-18).
- 22:00 1호 차량이 두솔동에서 보드 1 · 헬멧 1 · 권 1매를 받고(`stock.collect`), 기사가 차량 지갑에서 5,000원을 돌려드린다(`field.deposit_return`: 오프라인이면 보냄 대기에 쌓이고 돈이라 거절되지 않음): `deposit_out`(차량 지갑, `collected_by_vehicle_id`) + 돌려드림 줄(수거 이동 줄). 박준호 팀 보관 0매 · 0원.
- 22:10 최하은 팀 권 1매 수거 실패. 26일 마감(27일 00:40): 카운터 돈통 예상 = 시재 100,000 + 현금 수납 405,000 + 보증금 받음 35,000 − 보증금 돌려드림 30,000 = 510,000원, 1호 차량 지갑 = 35,000 + 5,000 − 5,000 = 35,000원. 00:32에 차량 현금 35,000원을 세고 확인하면 카운터 돈통 예상은 545,000원이 되고(마감 화면이 보이는 값, 4-13), 00:36 돈통 점검 545,000원 · 차액 0원. `보증금 보관 중 · 리프트권 1매 · 5,000원`, 인계 `deposit`(최하은 · 5,000원 · 1매)과 `return`(지연). 보증금은 수납 합계 1,965,000원에 들지 않는다.
- 27일 마감: 규칙의 하루가 차서 미리 보기에 `최하은 · 0026 권 1매 보증금 5,000원 · 마감 시 몰수`, 마감 명령이 폐기 이동(손님 → 폐기·분실) + `deposit_forfeit` 5,000 + 장부 `keep` 줄(마감 id). 27일 `closing_totals`의 `deposit_forfeit`은 5,000원 · 1매. 권 줄은 미반납으로 끝나고 접수가 닫힌다.
- 28일에 최하은 팀이 권을 가져오면 분실 회수 이동(폐기·분실 → 매장) + `deposit_restore` + `deposit_out`. 돌려드릴지는 사람이 정한다(확인 필요 `deposit_kept_returned`).
