...
| 화면 문구 | [docs/design/wording.md](docs/design/wording.md): 한 개념 한 말의 단일 기준(원칙 1절, 기준표 1-2, 영역별 표 3절, 함께 고칠 곳 4절) |
...
- 화면 글은 포스 · 가게에서 쓰는 **짧은 업무 용어**다(2026-09-24 사용자 요청, docs/architecture/README D6). 나이 많은 사장님이 바로 읽는 말이고 한 개념은 한 말이다: 수령 · 지급 · 반납 · 수거 · 배달 · 적재 · 입고 · 수납 · 결제 · 미수 · 후불 · 반환 · 일정 · 완료 · 미처리 · 부분 · 대기 · 지연 · 잔여 · 전송 대기 · 긴급 · 차량 재고 · 현금 점검 · 이월. **기준은 [docs/design/wording.md](docs/design/wording.md) 하나다.** 표에 없는 말을 새로 만들지 않는다. 새 말이 필요하면 그 표에 먼저 넣고 사람에게 묻는다.
- 사용자가 받아들인 말은 그대로 둔다: 장부, 대여 접수증, 새 접수, 수거 목록, 매장 입고, 미수, 수납, 지급, 반납, 리프트권, 보증금, 확인 필요, 끝 4자리, 연결 끊김.
- **버튼 · 이름표 · 탭은 이름씨다.** 문장 끝과 설명(`~합니다` · `~드림` · `~주세요` · `~하기` · `~정하기` · `~고르기` · `~바꾸기`)을 쓰지 않는다(굳은 `찾기` · `더 보기` · `나가기`만 예외). 화면에 `약속`을 쓰지 않는다(`수령 일정` · `반납 일정`, 바꾸는 동작은 `일정 변경`).
- **동작 버튼은 동작 이름 + 수 · 금액이다:** `반납 처리 · 5개`, `수납 처리 · 120,000원`, `접수 확정 · 현금 160,000원`. 도장은 그림으로 남기고 버튼에 `도장 찍기` 같은 비유를 쓰지 않는다. 돌아가는 버튼은 가는 곳 이름이다(`‹ 장부`).
- **화면에 영어 글자가 없다**(검사기가 센다). 기술 용어(동기화, 서버, 오류 코드, 캐시, 데이터베이스 …)도 화면에 쓰지 않는다.
- AI가 쓴 것 같은 말투를 쓰지 않는다: 과장, 감탄, "~해 드릴게요!", "스마트한", 이모지.
- 날짜는 `오늘 · 내일 · 모레`로 쓰고 같은 날짜를 두 번 쓰지 않는다. 시각은 숫자(`06:00`), 돈은 `120,000원`.
- 막히는 일과 알림은 기술 설명 대신 **`사실 · 할 일` 한 줄**과 버튼 두 개다(`이 팀 120,000원 수납 완료 · 다른 카운터`, `처리 실패 · 재시도 필요`). 할 일 쪽도 이름씨로 끝내고, 버튼이 말하는 것은 다시 쓰지 않는다.
...
- 이름 뒤에 조사를 붙여 문장을 만들지 않는다(`label + '은 …'`는 받침에 따라 틀린다). 표시를 통째로 문구 표에 두고 이름은 제목이나 조사가 필요 없는 자리에 쓴다(`1호 차량 수거 예정 · 22:00`). 코드가 이름에 글을 이어 붙이는 곳(`label + ' 찍기'`)도 만들지 않는다.
...
- **부품은 업무 규칙을 계산하지 않는다.** 도장 상태, 다음 처리, 지연의 기준 시각, 받을 금액은 읽기 모델에 이미 들어 있고 부품은 그리기만 한다. 화면(`apps/pos/src/screens`)도 규칙을 만들지 않고 `DomainClient`(`ledgerView` · `query` · `command`)에 묻는다.
