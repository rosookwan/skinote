...
// 체험 자료는 첫 매장 모양이다(번호 · 권 보증금 없음, 2026-09-26). 주소의 ?shop=numbered는 번호 · 권 보증금을 켠 견본 매장으로 연다(화면 규칙
// 검사기가 보증금 줄 · 번호 버튼이 있는 창을 재는 길, 자료는 따로 저장).
...
/** 체험 자료의 견본 매장 모양(주소 ?shop=numbered면 번호 · 보증금 매장). */
const demoShop = (): 'first' | 'numbered' => (new URLSearchParams(window.location.search).get('shop') === 'numbered' ? 'numbered' : 'first');
