// 보증금(체험 전용, plan.md 4-1 · catalog 11-1 · data-model 4-12): 팀 · 규칙마다 보관 하나(규칙 값 복사), 돈은 보증금 장부
...
// LocalClient가 오면 폴더째 없어진다.
import type { CommandEnvelope, Expect } from '@skinote/contract';
import type { FxDeposit, FxDepositEntry, FxDepositRule, FxLine, FxMethodKey, FxOrder, FxState } from './model.ts';
...
/**
 * 이 줄에 걸리는 보증금 규칙(deposit_rules의 대상 종류 = 체험 자료에서는 규칙의 결제 칸, 이 매장 리프트권 보증금). 보증금 미사용이거나
 * 돌려받지 않는 줄이면 null.
 * 이미 받은 보관은 규칙 값을 복사해 두었으므로(FxDeposit) 규칙이 바뀌어도 그 팀에 말한 금액대로다.
 */
export function depositRuleFor(state: FxState, l: FxLine): FxDepositRule | null {
 ...
 */
export function heldRule(state: FxState): FxDepositRule | null {
  return state.settings.liftDeposit ?? state.settings.liftDepositOff ?? null;
}

export function depositOf(state: FxState, orderId: string, ruleKey: string): FxDeposit | undefined {
...
export function orderDepositHeld(state: FxState, o: FxOrder): number {
 ...
 */
export function depositDueAtIssue(state: FxState, o: FxOrder, picks: readonly { l: FxLine; qty: number }[]): DepositDue | null {
  const rule = state.settings.liftDeposit;
  if (!rule) return null;
 ...
 */
export function depositDueForIssued(state: FxState, o: FxOrder): DepositDue | null {
  const rule = state.settings.liftDeposit;
  if (!rule) return null;
  const dep = depositOf(state, o.id, rule.key);
...
export function backHeldRefund(state: FxState, o: FxOrder): { dep: FxDeposit; units: number; amount: number } | null {
 ...
 */
export function takeDeposit(state: FxState, envelope: CommandEnvelope<'deposit.take'>, now: number): Result {
 ...
 */
export function returnDeposit(state: FxState, envelope: CommandEnvelope<'deposit.return'>, now: number): Result {
  const p = envelope.payload;
  const o = state.orders.find((x) => x.id === p.orderId);
  if (!o) return rejected('접수 없음');
