...
export type LineStampFn = (o: FxOrder, l: FxLine, step: StampStepRow, steps: Steps, today: BizDay) => StampCell | null;

/** 줄 하나의 도장(줄 단위 규칙). null이면 그 단계가 이 줄에 해당 없음(na). */
export function lineStamp(o: FxOrder, l: FxLine, step: StampStepRow, steps: Steps, today: BizDay): StampCell | null {
  switch (step.rule_key) {
    ...
    case 'qty_returned': {
      ...
      if (van && open.every((b) => b.promise.mode === 'vehicle')) {
        const vehicle = vehicleLabel(van.promise.vehicleId);
 ...
 */
export function rollupStep(o: FxOrder, step: StampStepRow, steps: Steps, today: BizDay, lineFn: LineStampFn = lineStamp): StampCell {
  const cells = o.lines.map((l) => ({ line: l, cell: lineFn(o, l, step, steps, today) })).filter((x): x is { line: FxLine; cell: StampCell } => x.cell !== null);
...
export function stepStamp(state: FxState, o: FxOrder, step: StampStepRow, steps: Steps): StampCell {
  ...
  return rollupStep(o, step, steps, bizDay(state));
}
 ...
 */
export function driverLineStamp(o: FxOrder, l: FxLine, step: StampStepRow, steps: Steps, today: BizDay): StampCell | null {
  if (step.rule_key !== 'qty_issued' || !isVehiclePickup(o)) return lineStamp(o, l, step, steps, today);
...
export function driverColumnStamp(o: FxOrder, stepKeys: readonly string[], steps: Steps, today: BizDay): StampCell {
  let last: StampCell = { stepKey: stepKeys.at(-1) ?? '', state: 'na' };
  for (const key of stepKeys) {
    const step = steps.get(key);
    if (!step) continue;
    let cell = rollupStep(o, step, steps, today, driverLineStamp);
 ...
 */
export function checklist(state: FxState, o: FxOrder, steps: Steps, slipStepKeys: readonly string[], today: BizDay): { items: ChecklistItem[]; next: NextStep | null } {
  ...
  for (const key of keys) {
    ...
    switch (step.rule_key) {
      ...
      case 'qty_returned': {
        ...
        const first = list[0]!;
        const where = (p: typeof first) => (p.mode === 'store' ? '매장' : placeLabel(p.placeId));
