...
import type { FxLine, FxOrder, FxPin, FxReturnSlot, FxState } from './model.ts';
...
export const bizDay = (state: FxState): BizDay => ({ date: state.businessDate, cutoff: shopCutoff(state.settings) });
...
export function lastReturnSlotAt(state: FxState, date: string): number {
 ...
 * 없다). 없으면 undefined.
 */
export function nightPrepSlotAt(state: FxState, date: string): number | undefined {
...
export function promisedPayer(state: FxState, o: FxOrder): FxOrder | undefined {
...
/** 이 팀이 대신 낼 몫의 합(대납). */
export const othersDue = (state: FxState, o: FxOrder) => coveredOrders(state, o).reduce((sum, other) => sum + dueFor(other, o.id), 0);
 ...
 */
export function routeTasks(state: FxState, vehicleId: string, date: string): FxTask[] {
 ...
 */
export function listTasks(state: FxState, vehicleId: string, date: string): FxTask[] {
...
export function pinTask(state: FxState, pin: FxPin): FxTask | undefined {
...
export const pinTaskId = (state: FxState, pin: FxPin) => pinTask(state, pin)?.id ?? pin.taskId ?? 'collect:' + pin.orderId;
 ...
 */
export function sortTasks(state: FxState, tasks: readonly FxTask[]): FxTask[] {
...
/** 방문 결과 이름(reason_codes의 visit_result 체험 값). 업무 종류마다 셋을 보인다(visitReasonKeys). */
 ...
 */
export const VISIT_REASON_KEYS: Readonly<Record<'deliver' | 'collect', readonly (keyof typeof VISIT_OUTCOMES)[]>> = {
  collect: ['customer_absent', 'place_changed', 'items_not_ready'],
  deliver: ['customer_absent', 'place_changed', 'other'],
};

export const visitReasons = (kind: 'deliver' | 'collect') => VISIT_REASON_KEYS[kind].map((key) => ({ key, label: VISIT_OUTCOMES[key] }));
