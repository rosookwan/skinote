// 번호로 세는 실물(스키 17번 · 야간권 31번)의 번호 고르기(체험 전용, plan.md 4-1). 줄의 수 셈(issued · returned · collected)이
...
// 운영에서는 store · domain이 장비 위치(assets · 이동 줄)로 한다. LocalClient가 오면 폴더째 없어진다.
import type { FxLine, FxState } from './model.ts';
 ...
 */
export function freeNumbers(state: FxState, kind: string, forLine?: FxLine): string[] {
 ...
 */
export function issueNumbers(state: FxState, l: FxLine, qty: number, requested?: readonly string[], taken: ReadonlySet<string> = new Set()): NumberPick {
 ...
 */
export function loadNumbers(state: FxState, l: FxLine, qty: number, requested?: readonly string[], taken: ReadonlySet<string> = new Set()): NumberPick {
  if (!numbered(l) || qty <= 0) return { ids: [] };
