...
import type { FxLine, FxMethodKey, FxOrder, FxState, FxVisitOutcome } from './model.ts';
...
import {
  backCount, collectDone, fillLines, findOrder, isDeliverTaskId, isVehiclePickup, openPins, orderIdOfTask, ownDue, pendingIssue, pinTaskId, routeTasks,
  selfDue, selfLines, slotKey, sortTasks, VISIT_OUTCOMES,
} from './rules.ts';
...
const isMethod = (key: string): key is FxMethodKey => payMethodOf(key) !== undefined;
  ...
  switch (envelope.type) {
    ...
    case 'payment.take': {
      ...
      targets.forEach((o, i) => {
        ...
        o.payments.push({ id: envelope.requestId + ':' + i, amount: share, methodKey, at: now, groupId: envelope.requestId, ...drawer, ...(lines.length ? { lines } : {}) });
        rest -= share;
      });
      return done;
    }
    ...
    case 'promise.change':
      ...
      return changePromise(state, envelope, now);
    case 'deposit.return':
    ...
    case 'order.create':
      ...
      return createOrder(state, envelope, now);
    case 'field.collect':
    ...
    case 'field.add_ticket':
      ...
      return addTicket(state, envelope, now);
    ...
    case 'payment_promise.set':
      ...
      return setPaymentPromise(state, envelope, now);
    case 'cash.transfer':
      // 차량 현금 인계(매장 입고 때 기사가 봉투를 건넴, 이야기 23:48, closing.ts).
      return transferCash(state, envelope, now);
    case 'cash.transfer_confirm':
      // 마감 화면의 차량 현금 점검(V6, 넘기는 중 → 카운터 돈통, closing.ts).
      return confirmTransfer(state, envelope, now);
    case 'closing.close':
      // 하루 마감(V6, closing.ts).
      return closeDay(state, envelope, now);
    case 'setting.set':
      // 운영 규칙 저장(V8, 다음 기록부터, shop-rules.ts).
      return applySettings(state, envelope, now);
    default:
 ...
 * 적재(카운터가 적음) · 후불 처리(결제 약속은 결정)는 연결이 필요하다. 체험판의 기사 기기(1호 차량 태블릿 · 휴대폰)는 같은 목록이다.
 */
...
function unmetDependency(state: FxState, dependsOn: readonly string[] | undefined): boolean {
