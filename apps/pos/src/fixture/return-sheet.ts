// V1 반납 확인 창 · 부분 반납(spec 3-1, ui 6-2, data-model 4-18)의 읽기 모델(returnSheet). 체험 전용이고 LocalClient가 오면 폴더째
// 없어진다. 창은 처음에 이 팀이 가진 것을 모두 골라 두고(번호로 세는 품목은 번호, 수량 품목은 수량), 사람이 고른 것(번호 · 수량 ·
...
import type { FxDeposit, FxLine, FxOrder, FxPromise, FxState } from './model.ts';
...
const noOf = (state: FxState, id: string) => state.assets.find((a) => a.id === id)?.no ?? id.slice(id.lastIndexOf('-') + 1);
...
const shortName = (p: FxPromise) => (p.mode === 'vehicle' ? placeShortLabel(p.placeId) : '매장');

/**
 * 칸 둘째 줄: 번호 칸은 일정별 수(`설천 2 · 두솔동 1`, 일정이 하나면 `설천 2대`), 수량 칸은 `대여 6개`. `N개 중`은 쓰지 않는다
 * (헬멧 규격 `중`과 헷갈림). 짧은 이름이 겹치면 온전한 장소 이름.
 */
function lineNote(o: FxOrder, l: FxLine, mode: Mode): string {
  const held = heldOf(l);
  if (mode === 'count') return '대여 ' + held + countWordOf(l);
  const open = lineBuckets(o, l).filter((b) => b.planned > 0 && bucketOut(b) > 0);
  if (open.length <= 1) return shortName(open[0]?.promise ?? o.giveBack) + ' ' + held + countWordOf(l);
  const short = open.map((b) => shortName(b.promise));
  const names = new Set(short).size === short.length ? short : open.map((b) => placeText(b.promise));
...
function promiseLeft(state: FxState, p: FxPromise, now: number): string {
  const when = whenText(state, p.at);
  if (p.mode === 'vehicle') return p.at > now ? when + ' ' + placeText(p) + ' 수거 예정' : placeText(p) + ' 수거 · 일정 ' + when;
 ...
 */
function remainderOf(state: FxState, o: FxOrder, picks: readonly Pick[], now: number): RichText {
  ...
  const cancelRun: TextRun[] = cancelled.length ? [{ text: ' · ' + cancelled.map((g) => placeText(g.promise)).join(' · ') + ' 수거 취소' }] : [];
 ...
 */
function refundOf(state: FxState, o: FxOrder, all: readonly FxLine[], picks: readonly Pick[]): Refund | null {
 ...
 */
function refundMethods(state: FxState, o: FxOrder, r: Refund, wanted: string | undefined): { options: ChoiceOption[]; selected: RefundMethodKey | null } {
...
export function returnSheet(ctx: ViewContext, params: ReturnSheetParams): ReturnSheetView {
  ...
  const lines: ReturnPieceLine[] = picks.filter((p) => p.mode !== 'none').map((p): ReturnPieceLine => {
    ...
    const names = lineNames(p.l);
    const base = {
      lineId: p.l.id, label: names.name, note: [names.variant, lineNote(o, p.l, p.mode)].filter(Boolean).join(' · '), muted: p.n === 0,
