...
import { blockedMessage, checklist, columnStamp, deliverLateAt, lineStamp, returnLateAt, stepStamp } from './stamps.ts';
...
/** 반납 일정: '22:00 설천 주차장 · 차량', '16:30 매장', '내일 09:00 솔마을 한솔동 · 차량'. */
...
const isRevisit = (o: FxOrder) => o.visits?.at(-1)?.retryAt === o.giveBack.at;
...
/** 차량 재고: 품목별 합(수거했지만 매장에 아직 입고하지 않은 것). */
...
export function collectionList(ctx: ViewContext, params: ViewParams): LedgerViewResult {
  ...
  const revisitSlots = new Set(tasks.filter((o) => tasks.filter((x) => slotKey(x) === slotKey(o)).every(isRevisit)).map(slotKey));
  const groups = [...new Map(shown.map((o) => [slotKey(o), o.giveBack.at] as const)).entries()].map(([key, at]) => {
    ...
    return { key, label: hm(at) + ' ' + (revisitSlots.has(key) ? '재방문' : '반납'), shortLabel: hm(at), at: iso(at), done: inSlot.filter(collectDone).length, total: inSlot.length };
  });
  const rows: LedgerRow[] = shown.map((o, i) => {
    ...
    return {
      ...
      ...(byShop ? { reviewNote: '매장 반납' } : !done && isRevisit(o) && !revisitSlots.has(slotKey(o)) ? { reviewNote: '재방문' } : {}),
    };
  });
  ...
  const pins: PinRow[] = openPins(ctx.state).flatMap((p) => {
    ...
    return [{
      ...
      parts: [
        { text: placeLabel(o.giveBack.placeId), short: placeShortLabel(o.giveBack.placeId), drop: 1 },
...
export function reviewList(ctx: ViewContext): ReviewItem[] {
  ...
  for (const o of ctx.state.orders) {
    ...
    if (due?.kind === 'return' && due.lateAt !== undefined && due.lateAt <= ctx.now) {
      out.push({
        ...
        message: o.teamName + ' 팀 반납 지연 · ' + when(due.at, ctx.state.businessDate) + ' ' + (o.giveBack.mode === 'store' ? '매장' : placeLabel(o.giveBack.placeId)),
      });
    }
  }
  for (const p of openPins(ctx.state)) {
    ...
    out.push({
      ...
      message: o.teamName + ' 팀 ' + (p.note ? p.note + ' · ' : '') + hm(p.at) + ' ' + placeLabel(o.giveBack.placeId) + ' · 긴급 요청',
    });
  }
  for (const o of ctx.state.orders) {
    ...
    out.push({
      ...
      message: o.teamName + ' 팀 ' + VISIT_OUTCOMES[visit.outcomeKey] + (visit.retryAt !== undefined ? ' · 재방문 ' + when(visit.retryAt, ctx.state.businessDate) : ''),
    });
  }
  return out;
}
...
function receiveDraft(ctx: ViewContext, vehicleId: string): ConfirmDraftView {
  ...
  return {
    ...
    title,
    summary: [[tasks.length + '팀', ...items.map((it) => it.label + ' ' + it.qty)].join(' · ')],
...
export function confirmDraft(ctx: ViewContext, params: ConfirmDraftParams): ConfirmDraftView {
  ...
  switch (params.actionKey) {
    case 'stamp.collect':
      return stockDraft(ctx, o, params, label('stamp.collect'), collectLeft,
        collectDone(o) ? '수거 완료' : '수거 대상 없음',
        (picks) => ({ type: 'stock.collect', payload: { taskId: collectTaskId(o), lines: lines(picks) } }));
    case 'pin': {
      const title = label('pin') + ' · ' + o.teamName + ' 팀';
      if (!isVehicleReturn(o) || !anyIssued(o) || collectDone(o)) return notice(ctx, title, '차량 수거 대상 없음');
      if (kstDate(o.giveBack.at) !== ctx.state.businessDate) return notice(ctx, title, when(o.giveBack.at, ctx.state.businessDate) + ' 수거 · 오늘 목록 제외');
      ...
      const note = o.giveBack.note;
      return {
        basis: head(ctx).basis,
        title,
        summary: [
          [hm(o.giveBack.at) + ' ' + placeLabel(o.giveBack.placeId), o.teamName, o.last4, ...(note ? [note] : [])].join(' · '),
          vehicleLabel(o.giveBack.vehicleId) + ' 수거 목록 맨 위 고정 · 기사 알림',
        ],
        confirmLabel: label('pin'),
        command: { type: 'task.pin', payload: { taskId: collectTaskId(o), ...(note ? { note } : {}) } },
      };
    }
    case 'stamp.issue':
      return stockDraft(ctx, o, params, label('stamp.issue'), (l) => l.qty - l.issued, '지급 완료',
        (picks) => ({ type: 'stock.issue', payload: { orderId: o.id, lines: lines(picks) } }));
    case 'stamp.return':
      return stockDraft(ctx, o, params, label('stamp.return'), (l) => (l.returnable ? l.issued - backCount(l) : 0),
    ...
    case 'pay': {
      ...
      const summary = covered.length > 0
        ? [...(ownDue(o) > 0 ? ['미수 ' + won(ownDue(o))] : []), ...covered.map((t) => t.teamName + ' 팀분 대납 ' + won(ownDue(t))), '받을 금액 ' + won(total)]
        : payer
