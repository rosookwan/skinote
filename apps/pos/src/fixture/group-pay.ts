// V5 일괄 수납 · 여러 팀(spec 3-6, ui 6-7)의 읽기 모델(groupPaySheet · partialPaySheet)과 일괄 수납 명령(payment.take + 결제 팀)의 체험
// 처리. 체험 전용이고 LocalClient가 오면 폴더째 없어진다. 화면은 고른 팀 · 탭 · 팀마다 고른 품목 수 · 수단 · 더한 팀만 인자로 다시 묻고,
...
import { PAY_METHODS, lineNames, payMethodOf } from './catalog.ts';
import type { FxLine, FxMethodKey, FxOrder, FxPaymentGroup, FxState } from './model.ts';
...
export function groupPayPlan(state: FxState, params: GroupPaySheetParams): GroupPayPlan {
  ...
  const method = params.methodKey === undefined ? payMethodOf(DEFAULT_METHOD) : payMethodOf(params.methodKey);
  return {
...
export function groupPaySheet(ctx: ViewContext, params: GroupPaySheetParams): GroupPaySheetView {
  ...
  const method = payMethodOf(plan.methodKey);
  const nonQuick = method !== undefined && !method.quick;
  const methods: ChoiceOption[] = [
    ...PAY_METHODS.filter((m) => m.quick).map((m) => ({ key: m.key, label: m.label, selected: plan.methodKey === m.key, enabled: true })),
    { key: CHECKOUT_KEYS.other, label: '기타', selected: nonQuick, enabled: true, ...(nonQuick ? { secondLine: method.label } : {}) },
  ];
  const others: ChoiceOption[] = PAY_METHODS.filter((m) => !m.quick).map((m) => ({ key: m.key, label: m.label, selected: plan.methodKey === m.key, enabled: true }));
 ...
 */
export function takeGroupPayment(state: FxState, envelope: CommandEnvelope<'payment.take'>, now: number): Result {
  ...
  if (!payMethodOf(p.methodKey)) return rejected('등록되지 않은 결제 수단');
  ...
  plan.selected.forEach((row, i) => {
    ...
    row.order.payments.push({
      id: envelope.requestId + ':' + i, amount: share.amount, methodKey: plan.methodKey, at: now, groupId: envelope.requestId, lines: share.lines, ...drawer,
    });
  });
