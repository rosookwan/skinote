...
import { PRODUCTION_LINES, applyCommand as applyDomainCommand, kstAt, sampleDay, type DomainLines, type SampleShop, type ShopState } from '@skinote/domain';
...
/**
 * 처음 자료(12월 26일 15:40의 18팀). epoch는 되돌릴 때마다 새로 받는다. shop: 체험판은 첫 매장(수량 · 보증금 없음), 시험은 번호 · 보증금을
 * 켠 매장(numbered)도 쓴다.
 */
export const createSeed = (epoch: string, shop: SampleShop = 'first'): ShopState => sampleDay({ date: DEMO_DATE, epoch, ids: 'demo', shop });
