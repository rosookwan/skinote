// V9 일정 변경 · 부분 품목(spec 3-2, ui 6-2 N2)의 읽기 모델(promiseSheet)과 명령(promise.change). 체험 전용이고 LocalClient가 오면
// 폴더째 없어진다. 일정은 줄의 수량에 붙고, 수량 일부를 떼어 새 일정을 만든다(promises.ts의 일정 몫). 어느 장비인지는 반납 · 수거 때
...
import type { FxCharge, FxLine, FxOrder, FxPromise, FxPromiseSplit, FxReturnSlot, FxState } from './model.ts';
...
const cutoffOf = (state: FxState) => shopCutoff(state.settings);
const bizDate = (state: FxState, at: number) => businessDateOf(at, cutoffOf(state));
...
/** 고른 날의 영업일. */
export function dateOf(state: FxState, day: SlotPick['day']): string {
...
export function timeText(state: FxState, at: number): string {
  const text = hm(at);
...
export function whenText(state: FxState, at: number): string {
...
export const placeText = (p: FxPromise) => (p.mode === 'vehicle' ? placeLabel(p.placeId) : '매장');
/** 일정 한 줄('22:00 설천 주차장'). */
const promiseText = (state: FxState, p: FxPromise) => whenText(state, p.at) + ' ' + placeText(p);
...
function pickOf(state: FxState, p: FxPromise): { slot: SlotPick; place: PlacePick } {
...
export const knownPlace = (key: string | undefined) => AREAS.some((a) => a.places.some((p) => p.id === key));

/**
 * 일정 입력(PromiseInput)을 일정으로. 반납 타임이 없거나 ''이면 source의 시각을 그 날로 옮긴다. 매장 설정에 없는 반납 타임 · 장소 ·
 * 차량은 창이 만들 수 없는 입력이라 새 문구 없이 '체험판 미지원'으로 거절한다(문구 표에 없는 말을 만들지 않는다).
 */
function resolvePromise(state: FxState, input: PromiseInput, source: FxPromise): FxPromise | { error: string } {
  let at = source.at;
  if (input.slot) {
  ...
  if (!knownPlace(input.placeKey)) return { error: UNKNOWN };
  const vehicleId = input.vehicleId ?? source.vehicleId ?? VEHICLES[0]?.id;
  if (!VEHICLES.some((v) => v.id === vehicleId)) return { error: UNKNOWN };
...
const dayPrice = (l: FxLine) => productOf(l.productKey ?? l.kind)?.price ?? (l.qty > 0 ? l.amount / l.qty : 0);
 ...
 */
function extensionOf(state: FxState, o: FxOrder, moves: readonly LineMove[], target: FxPromise, now: number): { amount: number; parts: Omit<FxCharge, 'id' | 'at'>[] } {
  const after = nextSplits(o, moves, target, now);
  const mainDate = bizDate(state, o.giveBack.at);
  const parts: Omit<FxCharge, 'id' | 'at'>[] = [];
  for (const m of moves) {
    ...
    const want = Math.round(dayPrice(m.l) * unitDays);
    const had = (o.charges ?? []).filter((c) => c.kind === 'extension' && c.lineId === m.l.id).reduce((sum, c) => sum + c.amount, 0);
    const hadDays = (o.charges ?? []).filter((c) => c.kind === 'extension' && c.lineId === m.l.id).reduce((sum, c) => sum + c.days * c.quantity, 0);
    if (want === had) continue;
    const days = m.moving > 0 ? Math.round((unitDays - hadDays) / m.moving) : 0;
...
const liftDayChange = (state: FxState, moves: readonly LineMove[], target: FxPromise) =>
...
function lineNote(o: FxOrder, l: FxLine): string {
  const open = lineBuckets(o, l).filter((b) => b.planned > 0 && bucketLeft(b) > 0);
  if (open.length > 1) {
    return open.map((b) => (b.promise.mode === 'vehicle' ? placeShortLabel(b.promise.placeId) : '매장') + ' ' + bucketLeft(b)).join(' · ');
  }
...
export function promiseSheet(ctx: ViewContext, params: PromiseSheetParams): PromiseSheetView {
  ...
  const vehicleId = params.vehicleId ?? base.vehicleId ?? VEHICLES[0]?.id ?? '';
  ...
  const target = 'error' in resolved ? null : resolved;
  ...
  // ③ ④ 장소 · 차량.
  const areas = AREAS.map((a) => ({ key: a.id, label: a.label, lodging: a.lodging, places: a.places.map((p) => ({ key: p.id, label: p.label })) }));
  const vehicles: ChoiceOption[] = VEHICLES.map((v) => ({ key: v.id, label: v.label, selected: place.mode === 'vehicle' && vehicleId === v.id, enabled: true }));
  ...
  const lines = baseLines.map(({ l, left }) => ({
    lineId: l.id, label: l.label, note: lineNote(o, l), quantity: { value: qty(l.id, left), min: 0, max: left, unit: countWordOf(l) },
  }));
  ...
  const vehicleOf = (p: FxPromise) => (p.mode === 'vehicle' ? [{ text: ' · ' + vehicleLabel(p.vehicleId) }] : []);
  ...
  const changed: RichText = target && moves.length
    ? [
      ...
      { text: whenText(state, target.at) + ' ' + (target.mode === 'vehicle' ? placeLabel(target.placeId) : '매장 직접 반납'), strong: true },
      ...vehicleOf(target),
 ...
 */
export function changePromise(state: FxState, envelope: CommandEnvelope<'promise.change'>, now: number): Result {
  const p = envelope.payload;
  const o = findOrder(state, p.orderId);
  if (!o) return rejected('접수 없음');
  // 수령 일정 변경은 체험 자료에 없다(V9는 반납 일정).
  if (p.kind !== 'return') return rejected('체험판 미지원');
  ...
  for (const entry of p.lines) {
    ...
    if (move.moving < want) {
      ...
      return conflict('PROMISE_CHANGED', van ? vehicleLabel(van.promise.vehicleId) + ' ' + pieceText(l, van.collected) + ' 수거 완료 · 일정 확인' : '반납 완료');
    }
    moves.push(move);
  }
