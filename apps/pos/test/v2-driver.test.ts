...
// 시안의 순간은 16:55(최하은 팀, 16:40 적재), 뒷이야기 16:57 배달 · 16:58 권 추가 · 22:00 · 22:10 차량 수거(spec 2-4).
import {
  envelopeFor,
...
describe('V7 업무 판(taskSheet) — 16:55 최하은 팀', () => {
  it('시안의 글: 제목 · 팀 · 품목 표(적재 16:40 · 배달 미처리) · 돈 줄 · 큰 버튼 넷 · 반납 일정 · 완료 0 · 잔여 1 · 배달 처리 · 6개', async () => {
    ...
    expect(view.actions.map((a) => [a.actionKey, a.label, a.secondLine ?? null, a.enabled])).toEqual([
      ['field_collect', '현장 수납', null, true], ['add_ticket', '리프트권 추가', '야간권 재고 6매', true],
...
describe('배달 처리(stock.deliver)', () => {
  it('확인 창 `배달 처리 · 최하은 팀` · `배달 처리 · 6개` → 실은 번호를 지급, 업무 판 `배달 완료 · 16:55` · 주 버튼 누를 수 없음, 카운터 장부는 지급 완료', async () => {
    ...
    // 적재한 번호(준비 번호 맨 앞)를 건넸다.
    expect(o.lines.map((l) => l.assetIds)).toEqual(planned.map((ids) => ids.slice(0, 3)));
    const view = await client.query('taskSheet', { taskId: 'deliver:o26' });
...
describe('리프트권 추가 → 현장 수납(field.add_ticket · field.collect)', () => {
  ...
  it('권 줄(지급 · 반납 필수) · 보증금 입금(차량 지갑) → 현장 수납 판(미수 35,000원 · 후불 처리) → 수납(dependsOn) 현금 35,000원(결제 자리 driver_field · 차량 지갑)', async () => {
    ...
    expect(spareTickets(state(), 'v1')).toEqual([{ productKey: 'night_adult', ids: ['night_adult-52', 'night_adult-53', 'night_adult-54', 'night_adult-55', 'night_adult-56'] }]);
  ...
  it('현장 수납 판: 미수가 없으면 받을 돈이 없다(넣은 금액도 누를 수 없음), 받을 돈보다 많이는 받지 않는다', async () => {
    ...
    const typed = await client.query('fieldPaySheet', { ...withAmount({ taskId: 'deliver:o26', methodKey: 'card' }, '10000') });
  ...
  it('후불 처리(payment_promise.set): 이 팀 미수를 반납 때 받기로(늦음은 반납 일정 30분 뒤), 판에서 후불 처리가 사라진다', async () => {
    const { client, state } = at(75);
    const sheet = await client.query('addTicketSheet', { taskId: 'deliver:o26' });
    ...
    expect(slip.money).toMatchObject({ due: 35_000, lateAt: iso(22, 40) });
  });
});

describe('수거 업무 판 · 수거 현장의 권 보증금 반환(field.deposit_return)', () => {
  it('16:10 박준호 팀 수거 업무: 제목 `수거 · 22:00 설천 주차장`, 수거 칸, 돈 줄 둘째 줄 `권 3매 보증금 15,000원 반환`, 반납 일정 없음, 주 버튼 `수거 처리 · 6개 · 3매`', async () => {
    const { client } = at(30);
  ...
  it('수거 확인 창에 보증금 한 줄 → 수거 · 이어서 현장 반환 15,000원(차량 지갑 현금), 보관 0원', async () => {
...
describe('뒷이야기 16:40 ~ 22:10(plan.md 4-2)', () => {
  it('17:00: 16:57 배달, 16:58 권 추가(예비권 51번) · 보증금 5,000원 · 현장 수납 35,000원 현금(차량 지갑)', async () => {
  ...
  it('22:10: 22:00 1호 차량 수거(설천 주차장 · 두솔동 박준호 + 권 1매 보증금 5,000원 현장 반환), 22:10 최하은 장비만 · 정하늘 고객 부재 — 차량 지갑 35,000원, 맡은 보증금은 최하은 1매 5,000원뿐', async () => {
    const { client, state } = at(6 * 60 + 30);
    const s = state();
    ...
    expect(order(s, 'o41').visits).toEqual([{ at: ms(22, 10), outcomeKey: 'customer_absent', beforeAt: ms(22, 10) }]);
...
describe('업무 판의 크기(spec 3-8: 연결 띠를 늘 뺀 높이, DeviceProfile)', () => {
  ...
  it('휴대폰 360×640: 한 칸 세로에 품목 두 줄(누르는 곳 56 이상)이 들어간다, 넘치면 표 아래 쪽 넘김', () => {
    const room = taskRowRoom(phone, 640, { oneColumn: true, planLines: 1, pager: false });
    expect(room).toBe(120);
    expect(fillRows(room, 2, phone.fill.rowMinPx, phone.fill.rowMaxPx)).toEqual({ rowPx: 60, perPage: 2, pageCount: 1 });
    expect(fillRows(taskRowRoom(phone, 640, { oneColumn: true, planLines: 0, pager: true }), 4, phone.fill.rowMinPx, phone.fill.rowMaxPx).perPage).toBe(2);
  });
});
