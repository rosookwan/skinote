...
/** 체험 시계를 15:40에서 minutes만큼(뒷이야기 켬). 21:30 = 350분(16:05 박준호 지급 · 보증금, 19:41 일정 변경은 적혔고 21:31 반납은 아직). */
...
describe('V1 창의 읽기 모델(returnSheet) — 21:30 박준호 팀', () => {
...
describe('창의 고른 것 → 인자(ReturnDialog의 순수 함수)', () => {
...
describe('확정한 뒤(stock.direct_return + deposit.return)', () => {
...
describe('재고 방식별 칸(ui 3-1): 수량 칸 · 넓은 칸 · 번호 선택 작은 창', () => {
  it('수량으로 세는 줄은 −/+(둘째 줄 대여 N), 번호 4개부터 넓은 칸, 8개 넘으면 대여 N개 · 번호 선택 ›', async () => {
    ...
    expect(minsu.lines.map((l) => [l.label, l.mode, l.note, l.wide, l.quantity ?? null])).toEqual([
      ...
      ['헬멧', 'count', '대여 2개', false, { value: 2, min: 0, max: 2, unit: '개' }],
    ]);
...
describe('뒷이야기(plan.md 4-2): 21:31 부분 반납 · 21:32 수납 · 27일 00:15 정하늘 반납', () => {
  it('21:40: 박준호 팀 스키 17 · 18, 헬멧 12 · 14, 권 31 · 32 반납, 보증금 10,000원 현금 반환, 미수 120,000원 카드', async () => {
  ...
  it('사람이 21:30에 반납 창에서 먼저 했으면 21:31 사건은 건너뛴다(두 번 반납 · 두 번 반환하지 않음)', async () => {
    const { client, state } = at2130();
    const opened = await client.query('returnSheet', { orderId: 'o22' });
...
describe('같은 품목 말의 줄은 한 조각(점검 단계) — 21:10 이민호 팀(의류 사이즈 95 · 100 두 줄)', () => {
  it('반납 창의 미반납 줄 · 주 버튼과 일정 변경 창의 유지 줄에 `의류 2 · 의류 1`이 아니라 `의류 3`', async () => {
